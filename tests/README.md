...
| `test_workflow_security_contract.py` | Structural security invariants for the GitHub Actions surface, written after a red-team pass found it clean so the next workflow edit cannot quietly reintroduce a known Actions vulnerability class. Parses every workflow (and composite action) rather than grepping: no attacker-controlled expression (`github.event.*`, `github.head_ref`, `inputs.*`) may be interpolated into a `run:` script — the script-injection sink — while whole-line comments are exempt so a workflow can still *document* the footgun it avoids; no `pull_request_target`, which hands fork code a writable token and secrets; every workflow declares explicit `permissions` (or every one of its jobs does) and never `write-all`, so a change to the org-wide default can't silently widen a job; `persist-credentials: true` appears only in the two release stages that actually push a ref; every `runs-on` resolves — through `matrix.include` when expression-driven — to a known GitHub-hosted label, since a self-hosted runner would execute untrusted PR code on a persistent host; and `pages.yml`'s privileged `workflow_run` trigger keeps all four of its guards (success, `push` event, same repository, default branch). Three further invariants enforce supply-chain pinning across both workflows and composite actions: every third-party `uses:` names a 40-character commit SHA — a tag is a mutable pointer its publisher can move onto secret-reading code with no diff here, while local `./.github/actions/*` refs are exempt because they resolve inside the commit under review — and every pin keeps a trailing `# vX.Y.Z` comment, which is what Dependabot rewrites alongside the SHA and what makes the diff reviewable; the comment check is line-based (comments don't survive YAML parsing) and cross-checks its match count against the structural walk so a ref can't hide behind flow-style formatting. The third closes the loop on maintenance: `.github/dependabot.yml` must keep both its `/` entry (which for this ecosystem scans only `.github/workflows` plus a repository-root `action.yml`) and a `directories` glob reaching every composite action that names a third-party action, because a SHA nothing bumps is a pin that rots. Each invariant was mutation-tested: injecting a PR-title interpolation, a self-hosted label, a stray `persist-credentials`, a `pull_request_target` trigger, a reverted tag ref (in a workflow and in a composite action), a stripped version comment, a non-version comment, a flow-style `{uses: ...}` step, a deleted composite-action Dependabot block, a narrowed glob, and a moved `/` entry each fails the matching test. |
