...
| `test_job_submission_validation.py` | Accelerator-toleration requirement (GPU/Neuron/EFA jobs must carry a matching toleration) and the Job/CronJob parallelism cap — exercised against BOTH the REST `manifest_processor` and the SQS `queue_processor` so the SQS path is proven not to be a validation bypass |
