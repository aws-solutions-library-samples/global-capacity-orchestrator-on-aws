...
| `test_semantic_progress_doc_hygiene.py` | Static guard mirroring `test_metric_readers_doc_hygiene.py` — scans the feature's shipped files (`mcp/mission_judge/`, `mcp/tools/semantic_progress.py`, and the `test_semantic_progress_*.py` modules) and fails on any surviving spec breadcrumb (requirement IDs, `Validates:`, `Property N`, the feature tag, planning-doc filenames, spec prose). Self-excludes so its own pattern literals don't trip the check, and is written so the functional flag name `GCO_ENABLE_SEMANTIC_PROGRESS` and the `[gated by ...]` docstring prefix are never flagged. |
