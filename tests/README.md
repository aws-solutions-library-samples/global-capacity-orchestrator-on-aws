...
| `test_project_name_scoping.py` | Issue #139 project-name isolation — `ConfigLoader` `project_name` format validation, a backward-compat check that `project_name="gco"` still renders the pre-#139 physical names, and parametrized full-app synth over `project_name` permutations asserting every unique physical name (S3 buckets, CFN exports, WAF WebACL, auth secret, log groups, SSM params, DynamoDB tables, SageMaker domain, EMR app, Cognito domain) embeds the project name and that two deployments in the same account+region share zero unique names. Runs in the dedicated `unit:cdk:project-name-scoping` CI job. |
