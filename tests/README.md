...
| `test_live_validation_emulator.py` | `scripts/live_release_validation/emulator.py` + the amended `require_local_execution` — the single seam through which the local-only harness may run in CI, and only against a proven AWS emulator. Pins every fail-closed branch with mocked STS: https endpoints refused, non-allowlisted hosts refused, split `AWS_ENDPOINT_URL` refused, realistic (non-12-digit) credentials refused, identity-echo mismatch refused, echo match accepted; and `require_local_execution` staying a no-op outside CI, refusing CI without the emulator env, and routing CI-with-emulator through the verification. The happy path against a real emulator lives in `test_floci_live_validation_e2e.py`. |
