...
| `test_inference.py` | `InferenceManager` deploy / list / describe / scale / delete (CLI surface) and `InferenceMonitor` reconciliation loop (Deployment / internal Service lifecycle, legacy Ingress cleanup, complete classic and Mooncake teardown, status reporting, leader-election bootstrap). |
...
| `test_inference_canary_monitor.py` | Canary deployment reconciliation — image change detection, replica scaling, readiness status used by authenticated proxy weighting, historical direct-Ingress cleanup, canary cleanup, plus the capacity-type node-selector matrix (spot / on-demand / unspecified) on `_create_deployment`. |
...
| `test_regional_stack_feature_gap_coverage.py` | Feature-gap coverage for optional regional services and Helm workflows. It synthesizes real CDK constructs while mocking Docker assets and AWS lookups, then inspects their security, IAM scoping, convergence, and teardown lifecycle properties. |
