...
Static analysis tests act as guardrails against regressions in specific drift directions: Python-3.15 deprecation surface re-appearing in production code, spec / planning-document references leaking into production code or human-facing docs, and stale CVE suppressions in `.pip-audit-ignore` outliving their expiration date, and CI-config path references (CodeQL scan paths, coverage `--cov` targets, coverage source) outliving a package rename.
