...
| `test_cli_structured_output_contract.py` | Repository-wide JSON/YAML output transaction contract — native schema preservation, empty/scalar/raw envelopes, strict JSON constants, machine-safe prompts and eager options, table/nonzero compatibility, inherited-FD capture, concurrency and nested restoration, CRLF/non-UTF-8 handling, mission/swarm native documents, and static ownership/interaction guards across all registered CLI subsystems. |
...
| `test_validate_demo_gifs.py` | Tests for `.github/scripts/validate_demo_gifs.py`, which treats every tracked demo GIF as untrusted binary input and parses the block structure itself — before Pillow ever sees the file — so a disguised or malformed asset is rejected by a bounded hand-written parser rather than by whatever the decoder does with it. That parser is tested exhaustively, one byte-level malformation per test, on real Pillow-written GIFs mutated at known offsets: foreign or missing signature (a PNG with a `.gif` name fails at byte six), zero-size canvas, truncated global or local colour table, missing trailer, bytes appended after the trailer (where a payload would hide), a trailer before any frame, an unknown block marker, a truncated image descriptor, an empty or canvas-overflowing frame rectangle (a classic decoder crasher), a missing or out-of-range LZW code size, a truncated extension label, and data sub-blocks that overrun the file or never terminate. Fixtures are RGB rather than palette mode, because Pillow silently collapses look-alike palette frames into one and produced single-frame fixtures until that was caught. The allowlist is exercised against a throwaway git repository (missing, unlisted, and both), and per-file validation against the real decoder covers each policy ceiling, the symlink refusal, the autopilot-only blank-first-frame rule (scoped so a non-autopilot GIF may start blank), and every parser/decoder disagreement — format, canvas, frame count — as a rejection. `main()` maps a validation error, a git failure, and Pillow's decompression-bomb warning each to exit 1. One assertion reads the repository, and only via `git ls-files`: the committed allowlist must match the tracked GIFs. The decoded-pixel-budget branch carries a justified `pragma: no cover` — once each factor is bounded by its own ceiling the product cannot exceed the product of the ceilings, so it exists purely as defence in depth against loosening one check later. |
| `test_verify_inference_streaming_bundle_freshness.py` | Tests for `.github/scripts/verify_inference_streaming_bundle_freshness.py`, the CI-only regression that damages the real git-ignored streaming-Lambda bundle and proves `StackManager.synth` / `diff` rebuild it with the pinned npm. The end-to-end run needs the real bundle and a Node toolchain, which is why it lives in `unit:cdk:synth`; what is testable here is the script's own logic. It runs against a synthetic project under `tmp_path` with exactly one seam faked — `StackManager._build_inference_streaming_proxy_lambda`, the npm step — replaced by a builder that produces the same end state through the repository's *real* staging-and-publish machinery (`_prepare_lambda_asset`), so every freshness manifest the script inspects is genuine. Covers marker discovery (one `package.json` per direct dependency; a dependency-free package is refused), transitive-marker selection (skips direct dependencies including scoped ones, refuses a flat tree), every `_assert_fresh` rejection (stale manifest, a package file the manifest was lied about, a missing marker), and the full `main()` sequence — two damages, two repairs, exactly one rebuild each — plus each assertion `main()` can trip when a repair or detection is wrong. Asserts the seam still exists so a rename in `cli/stacks.py` fails here rather than turning the suite into a test of a stub, that the `unit:cdk:synth` workflow still invokes the script, and that the script imports `cli` when run as a file from outside the repository root. |
...
| `test_generate_openapi.py` | `scripts/generate_openapi.py`'s command line with stand-in applications and a temporary output directory: write mode creates missing documents, rewrites stale ones and leaves current ones untouched while reporting each outcome; `--check` names every missing or stale document, prints the regeneration command on stderr, exits non-zero and never writes; `build_document` strips FastAPI's own documentation routes; `render` is deterministic; `load_apps` puts the repository root on `sys.path` exactly once. (`test_api_docs_coverage.py` runs the real `--check` against the committed documents.) |
| `test_mkdocs_hooks.py` | `scripts/mkdocs_hooks.py`, the wiki image-injection hook, registered on a real validated `MkDocsConfig` and dispatched through `PluginCollection.on_files` the way `mkdocs build` runs `hooks:` entries: every regular file in `images/` becomes a generated `assets/images/<name>` file served from the tracked path (no bytes copied), `README.md` and subdirectories are skipped, ordering is deterministic, and the incoming collection is extended in place. A final case runs against the repository's real `images/` directory. |
| `test_mcp_install_smoke.py` | `scripts/mcp_install_smoke.py`, executed with `runpy` against stand-in `mcp` and `gco_mcp` modules and a fake `sys.executable` so nothing is installed: the four checks (`gco_mcp` imported from site-packages, `run_mcp.main` callable, the PyPI `mcp` SDK not shadowed, the resolved `gco` executable next to the interpreter), the exit code and report shape when any check fails, the success report, and the namespace-package fallback. |
...
| `test_live_validation_runner_actions.py` | `scripts/live_release_validation/{runner,models,context,artifact_io,cli_args,__main__,inference_contract}.py` + `actions/` | The runner driven end to end against a throwaway repository root with a fake action registry: guaranteed cleanup plus reporting once runtime construction succeeds, checkpoint identity and resume rules, signal and `KeyboardInterrupt` exit codes, strict `cdk.json` parsing, owner-only artifact I/O fallbacks, CLI argument validation and the blocked recovery report, the `RunContext` Job-record state machine, and every action module's fail-closed error branches. Every AWS, git, subprocess, signal and clock boundary is faked. |
| `test_live_validation_checks.py` | `scripts/live_release_validation/checks/` | The reusable validation helpers at the function boundary with faked API transports, boto3 clients, kubectl runners and clocks: the crash-safe API-transport Job lifecycle and every UID/label ownership refusal, central-queue identity binding and terminal cleanup reconciliation, add-on convergence and health/metrics probe validation, the ELBv2 HTTPS-only Gateway contract, the three-layer policy readback, the OpenCost report journal, and the managed-inference lifecycle's ownership, command, readiness, HPA, inventory and absence-proof edge cases. |
| `test_live_validation_ownership.py` | `scripts/live_release_validation/ownership/` | The durable proof of what a run created and may destroy, pinned as fail-closed identity matching: stacks owned only through persisted prepared-change-set authority plus the exact ARN and run tag; log groups only when the name derives from a checkpointed stack resource with a stable observed generation and both authority tags; KMS keys only as exact `AWS::KMS::Key` stack resources whose live tag matches the run; the delegated log-cleanup helper stack, role, template hash and STS principal re-validated on every touch; ECR residue retained rather than deleted; EFS automatic-backup recovery points and deleted-table DynamoDB streams stripped only after the service proves the source is gone. |
| `test_live_validation_inventory_cleanup.py` | `scripts/live_release_validation/{inventory,cleanup}/` + `protected.py` | Every per-service inventory scanner claims a resource only on an explicit ownership signal and fails closed on records without an identity, reporting the unfiltered live authority separately from the owned subset; `protected.py` matches a pre-existing account resource only by exact physical identity in the right partition, Region and account, never by prefix; cleanup re-validates deletion authority against the live resource immediately before every destructive request, refuses identity or run-tag drift, retains ECR residue, and keeps every blocked log group and workload visible in the returned evidence. |
...
| `test_capture_scaffold_fixtures.py` | Hermetic contracts for `scripts/capture_scaffold_fixtures.py` — active text-generation filtering, inference-profile geography preference and model-line deduplication, existing-fixture skipping, explicit-model ordering, list-only zero-invocation behavior, the bounded discovery timeout, and semaphore-limited model concurrency. Catalog and capture seams are stubbed; no AWS credentials, network calls, or paid inference are used. |
| `test_scaffold_fixture_replay.py` | Cross-model regression net for the scaffolder pipeline. Replays every captured [Bedrock](https://docs.aws.amazon.com/bedrock/latest/userguide/what-is-bedrock.html) model response under `tests/fixtures/scaffold_responses/*.json` through `_parse_response` -> `_normalize_sampled_criteria` -> `validate_criteria` -> `_validate_sampled_criteria_context` and asserts the round-trip succeeds. Each test id is `<model_slug>::<directive_slug>` so a regression points directly at the (model, directive) pair that broke. New fixtures captured via `scripts/capture_scaffold_fixtures.py` automatically join the parametrize matrix on the next run; see `tests/fixtures/scaffold_responses/README.md` for the runbook. |
...
| `test_check_runner_images.py` | The runner-image drift check (`.github/scripts/check_runner_images.py`), which compares the `runs-on:` labels in the workflows against the catalog `actions/runner-images` publishes. The distinction the whole check rests on gets the most attention: a newer **GA** image is drift worth acting on, a newer **preview** image is only context — reporting a preview as actionable would push CI onto an unannounced platform. Also covers deprecated-label reporting (with and without a catalogued successor), the refusal to compare across image families or architectures (macOS is not an upgrade for Ubuntu; x64 is not one for arm64), preferring an explicit recommendation over a floating `-latest` label, label collection from both direct `runs-on:` values and `runner:` matrix values, and self-hosted labels being surfaced rather than guessed about. Because the catalog is parsed from Markdown, the realistic failure is a harmless-looking upstream edit yielding an empty catalog that reads as "no drift" — so every malformed shape is asserted to produce *nothing*, and `main()` to turn nothing into exit 2 with empty stdout. Network access goes through an injected opener, so the suite never reaches raw.githubusercontent.com. |
| `test_check_bash_coverage.py` | The shell-coverage gate (`.github/scripts/check_bash_coverage.py`), which turns the bashcov/SimpleCov resultset the BATS suite produces into a pass/fail verdict. Covers the decision logic directly against synthetic resultsets — absolute-path mapping onto the tracked inventory (longest suffix, unique-basename fallback, refusing an ambiguous basename), merging hits from several reported paths onto one script, the ratchet split, and each failure mode — plus the shapes that must fail *closed* rather than read as success: a report scoped to the wrong root, a script no suite executed, and a resultset whose structure changed under a gem bump. Also guards `[tool.bash-coverage] ratchet` itself (entries name real tracked `*.sh` files, stay sorted and unique, and never cover every script, which would make the job green while measuring nothing) and asserts script basenames are unique, since the mapping fallback depends on it. Needs no Ruby or bats: `evaluate()` and the parsing helpers take plain data. |
...
| `test_floci_central_queue.py` | Floci emulator layer: `central_queue_worker.process_queued_jobs_once` orchestrating one dispatch pass on real DynamoDB — queued-work discovery through the unified `region-status-work-index`, fenced claim and CLAIMED→APPLYING→PENDING transitions with the Kubernetes identity persisted, spot-price-gated deferral that consumes no apply budget and leaves the record claimable, and permanent apply failures persisted as FAILED with the bounded error; only the worker's two designed seams (the Kubernetes apply and the EC2 spot gate) are doubled. |
...
| `test_floci_sqs_submit_path.py` | Floci emulator layer: the producer half of the regional job queue and the produce→consume seam — `JobManager.submit_job_sqs` discovers the regional stack over real CloudFormation, reads its `JobQueueUrl` output, and sends the envelope over real SQS; pins the envelope schema (body keys plus `Priority`/`JobId` message attributes), namespace fallback-not-override semantics, the loud missing-stack error, and drains the exact produced message through the unmodified production consumer. |
...
| `test_backend_tls.py` | The strict private-root TLS transport shared by the proxy Lambdas (`lambda/tls-shared/backend_tls.py`): settings validation that fails closed on a missing server identity or trust parameter, bounded tuning values, trust-bundle sanity checks (no private keys, real PEM, TLS 1.2+ with hostname verification), the SSM-backed pool cache (TTL, bounded stale grace, retry throttling, double-checked locking under a simulated concurrent refresh) and the cold-start reset hook. Uses a self-signed root generated at import time, so no fixture certificate is committed. |
...
| `test_capture_monitoring_screenshots.py` | `scripts/capture_monitoring_screenshots.py` with `playwright.sync_api` replaced by an in-memory fake: one headless Chromium launch, a 1600x900 context carrying a proactive HTTP basic-auth header, kiosk-mode dashboard URLs with optional `from`/`to` overrides, the fixed panel-render wait, full-page screenshots, `browser.close()` even when a navigation fails, the unauthenticated OpenCost UI capture, and `main()`'s option forwarding, defaults, required password and failure reporting. No browser is launched and Grafana is never contacted. |
...
| `test_drift_detection_handler.py` | The scheduled drift-detection Lambda itself (`lambda/drift-detection/handler.py`): the STACK_NAME / SNS_TOPIC_ARN environment contract and the AWS_REGION fallback, the three outcomes (in sync publishes nothing, drifted publishes the server-side-filtered and paginated resource list, a failed detection is alerted as a failure), the poll loop's terminal-status exit and timeout warning, and SNS subject truncation to 100 characters. |
...
| `test_feature_enablement_overrides.py` | The `feature_enabled_overrides` CDK context (`gco/config/config_loader.py`): parsing/validation of the comma-or-list override value against the documented four feature keys (`aurora_pgvector`, `valkey`, `fsx_lustre`, `vector_store`), each getter's forced enablement under override (selective, non-clobbering of other settings, never weakening an explicit cdk.json enablement), defaults staying disabled without it, and unknown names failing at config read with the valid list. Uses the repository's real cdk.json context so required-field drift can't silently skip validation. |
...
| `test_enablement_overrides.py` | The user-facing `--enable` surface (`gco/enablement_overrides.py` + `gco stacks deploy/deploy-all/destroy/destroy-all`) that routes one operator request into the two run-scoped CDK context keys. Load-bearing cases are the lockstep guards: the module re-declares both canonical name sets so the CLI can validate a name without importing `aws_cdk`, so these assert set equality against `config_loader.FEATURE_OVERRIDE_KEYS` and `regional_stack._HELM_CHART_CONFIG_KEYS`, that every routed name is accepted by its own real parser, and that the two namespaces stay disjoint (what makes a single flag route unambiguously). Also covers splitting of repeated vs comma-joined values, whitespace and empty segments, byte-stable sorted output regardless of input order, unknown names reporting every offender with the combined valid list, and — through `CliRunner` — that all four lifecycle commands reach `set_extra_cdk_context`, that `destroy-all` carries the overrides too (omitting them would synthesize a graph without the forced-on resources and leave them billing), that the applied context is disclosed in output so a recording shows it, that omitting the flag registers no context at all, and that an unknown name exits 2 before any AWS call. |
