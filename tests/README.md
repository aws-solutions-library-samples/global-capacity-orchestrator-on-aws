...
| `test_cli_autopilot.py` | `gco autopilot` behavior through `CliRunner` with the install/exec boundaries mocked: Bedrock model resolution precedence, generated MCP-config shape (companions, checkout vs pinned-uvx `gco` entry, read-only EKS default), GCO MCP feature-flag / `--mcp-env` injection and validation, session resumption (`--continue` / `--resume` / interactive prompt gating), the lazy pinned Claude Code install flow, and lockstep guards keeping `cli/autopilot.py`, the `gco_mcp/README.md` companion tables, and the deps-scan extraction regexes in agreement. |
...
| `test_config_loader_properties.py` | Hypothesis properties over the `ConfigLoader` validation/merge layer (the fast complement to the curated synthesis matrix): in-range `traffic_dial` / Global Accelerator health-check / `historical` configs merge losslessly with sibling defaults preserved, and out-of-range or wrongly-typed values always raise `ConfigValidationError` — never a leaked `KeyError`/`TypeError`. Base context is the shipped `cdk.json`, so the valid starting point tracks the repo. |
...
| `test_volume_cleanup_orchestrated_integration.py` | End-to-end orchestrated EBS volume cleanup through the real `StackManager.destroy_orchestrated` barrier, `ClusterAbsenceVerifier`, and `VolumeCleanupService`, with journaling EKS/EC2 doubles and scripted worker completion gates. Covers parallel regional destruction whose workers finish out of order while per-target `ebs-volumes` publication stays deterministic, cleanup running after every worker and [EKS](https://docs.aws.amazon.com/eks/latest/userguide/what-is-eks.html) security-group watchdog but before the global stacks, sequential and parallel runs publishing identical outcomes, mixed regional success/failure with non-regional stacks excluded (a failed stack reaches neither EKS nor EC2 and its volumes survive), and partial-teardown retries that process only newly verified targets without repeating work or touching unrelated volumes. No AWS client is created. |

### Dynamic EBS Volume Cleanup Tests

Tests for the command-aware EBS volume cleanup stage that regional stack
destruction runs after cluster removal (the `cleanup-dynamic-ebs-volumes`
spec). Every test is offline: AWS is either mocked or replaced with in-memory
doubles, and the property tests use Hypothesis. The feature's 13 correctness
properties each have one dedicated property test carrying a
`Feature: cleanup-dynamic-ebs-volumes, Property N: ...` tag. Two related
modules live in adjacent sections: `test_volume_cleanup_orchestrated_integration.py`
(CDK Stack Tests) and `test_volume_cleanup_multi_target_property.py` (Other
Tests, Property 12).

| File | Description |
|------|-------------|
| `test_volume_cleanup_policy.py` | Command-aware policy and Click authorization tests — the full single/`destroy-all` matrix, `-y`/`--yes` alias equivalence, explicit retain override (including `destroy-all -y --retain-volumes`), redundant `destroy-all -y --delete-volumes`, the dedicated irreversible-delete prompt for interactive explicit delete, prompt ordering, and rejection of contradictory `--retain-volumes`/`--delete-volumes` before any manager or resource call. |
| `test_volume_cleanup_policy_property.py` | Hypothesis property test (Property 1): command-aware policy resolution is deterministic and safe across every command kind and flag combination — conflicts rejected, explicit retain wins, `destroy-all -y` without retain authorizes delete, explicit delete with `-y` authorizes delete, and every remaining no-policy invocation resolves to retain. |
| `test_volume_cleanup_help_docs_contract.py` | Help-text and operator-doc contract for the volume policy — both `destroy` and `destroy-all` expose `--retain-volumes`/`--delete-volumes` as flags with their approved command-specific semantics, neither the help examples nor `docs/CLI.md`/`README.md` imply `--delete-volumes` is required for `destroy-all -y` (the bare `-y` example is annotated as deleting eligible volumes), and the live-validation `--volume-scenario` registry and runbook stay in lockstep with the E2E command lines the shared resolver actually produces. |
| `test_volume_cleanup_target.py` | Exact regional EBS cleanup target resolution — only `<project>-<region>` stacks with a configured/authorized Region, valid partition, and exact cluster identity become targets; near-miss names, unknown/cross-partition Regions, non-regional stacks, and unresolved strict identities return `NOT_REGIONAL` or `BLOCKED` and make no EBS calls. |
| `test_volume_cleanup_absence.py` | Fail-closed EKS cluster-absence verification (`ClusterAbsenceVerifier`) — only an exact not-found establishes absence; present, malformed, unauthorized, and throttling-exhausted responses stay blocked with no EC2 activity, and the `ClusterAbsenceProof` binds stack/Region/cluster/time. |
| `test_volume_cleanup_models.py` | Immutable EBS cleanup outcome models — frozen snapshot/outcome/count/target dataclasses, deterministic JSON-compatible serialization, `volume_id` ordering, stable action/result fields, and counts derived only from final records. |
| `test_volume_cleanup_classification.py` | Offline AWS DTO normalization and pure initial safety classification — malformed/duplicate/out-of-Region/near-miss-tag rejection, ownership exactly by tag value `owned`, retain-policy retention of every snapshot, and a delete candidate only for authorized owned, available, detached volumes. |
| `test_volume_cleanup_discovery.py` | Complete, Region-scoped `DescribeVolumes` discovery — the exact `tag-key` filter, Region-specific client, paginated pages defensively re-scoped, empty discovery as a completed no-op, and fail-closed handling of incomplete pagination and duplicate/conflicting volume IDs. |
| `test_volume_cleanup_deletion.py` | Just-in-time safety recheck and `DeleteVolume` — re-describe by exact volume ID, revalidate identity/AZ/Region/owned-tag/available/detached immediately before deletion, map exact `InvalidVolume.NotFound` to idempotent success, and record every other error as a safe failure. |
| `test_volume_cleanup_service.py` | Per-volume isolation, retry idempotency, and aggregate status — processing continues after normalization/evaluation/absence/reporting/deletion errors, one final record per in-scope volume, and target status/success derived so authorized delete fails when an owned volume remains while non-owned safety retention can still complete. |
| `test_volume_cleanup_ec2_integration.py` | Mocked EKS/EC2 integration of the whole sequence — drives `ClusterAbsenceVerifier` and `VolumeCleanupService` end to end, asserting the AWS call order is absence proof, complete discovery, per-candidate recheck, then deletion, with continuation after failures and no unrelated-volume calls. |
| `test_volume_cleanup_reporting.py` | Shared EBS cleanup renderer and publication — identical serialized target and per-volume fields for single and orchestrated destruction, the continuing-storage-cost warning for retained volumes, and the stable `ebs-volumes` publication with reporting failure surfaced independently from stack success. |
| `test_volume_cleanup_report_contract.py` | Terminal-status, warning, reporting-completeness, and exit-status contract tests closing the gaps between the focused renderer and exit-status suites — every terminal status, missing required fields, non-owned retention, owned safety skips, and already-absent success. |
| `test_volume_cleanup_exit_status.py` | Command-level cleanup status and exit mapping — retain succeeds after complete reporting, authorized delete fails when an owned volume is skipped/failed, already-absent is success, non-owned tagged volumes are safely retained, and stack failure keeps its existing exit status. |
| `test_volume_cleanup_single_destroy.py` | Single-stack `gco stacks destroy` post-destroy cleanup wiring — cleanup runs only after a reconciled successful deletion (including already-absent retries), a non-regional stack produces no outcome, and the cleanup result stays separate from the stack result. |
| `test_volume_cleanup_destroy_all_command.py` | Click-level `gco stacks destroy-all` volume-cleanup wiring and exit status — one resolved request authorizes every exact regional target, published `ebs-volumes` outcomes are read back and aggregated, and three-attempt retry semantics and exit codes are preserved. |
| `test_volume_cleanup_retain_property.py` | Hypothesis property test (Property 2): the retain policy — including the explicit retain override on `destroy-all -y` — produces no delete candidate or delete request for any ownership/state/attachment combination. |
| `test_volume_cleanup_scope_property.py` | Hypothesis property test (Property 3): regional targeting and EBS discovery scope are exact — a volume enters an outcome iff the stack is an exact authorized regional target and the volume is in the target Region with the exact cluster tag key, and owned is true iff the tag value is exactly `owned`. |
| `test_volume_cleanup_delete_candidate_property.py` | Hypothesis property test (Property 4): initial deletion classification enforces every safety predicate — a delete candidate arises iff authorized, owned, `available`, and detached; every other snapshot is preserved with the applicable reason and a reporting failure cannot convert a non-owned snapshot into a candidate. |
| `test_volume_cleanup_recheck_property.py` | Hypothesis property test (Property 5): just-in-time changes prevent deletion — a delete request occurs iff identity, Region, owned tag, available state, and empty attachments remain unchanged; any changed safety fact yields a skipped safety-preserved outcome. |
| `test_volume_cleanup_isolation_property.py` | Hypothesis property test (Property 6): per-volume failures are isolated — every processable volume gets exactly one final or reporting-failure record and an injected error on one volume never stops processing the rest. |
| `test_volume_cleanup_not_found_property.py` | Hypothesis property test (Property 7): exact `InvalidVolume.NotFound` at recheck or delete is the only idempotent absence signal; every other service error never becomes an absence classification. |
| `test_volume_cleanup_repeat_property.py` | Hypothesis property test (Property 8): repeated cleanup is scope-preserving and idempotent — target volumes only move toward retained/delete-requested/absent, a completed second pass touches no unrelated volumes, and a prior safe result is never reversed. |
| `test_volume_cleanup_aggregate_property.py` | Hypothesis property test (Property 9): aggregate cleanup success reflects every volume failure — unsuccessful whenever an evaluation or required-field report fails or an authorized delete leaves an owned volume undeleted, and successful otherwise. |
| `test_volume_cleanup_record_property.py` | Hypothesis property test (Property 10): non-success records are actionable and complete — records rendered through single destroy or destroy-all carry the same required identity/capacity/state/tag/attachment/policy/authorization/action/result fields, and retained/skipped/failed/non-owned records also carry a non-empty reason and follow-up. |
| `test_volume_cleanup_counts_property.py` | Hypothesis property test (Property 11): derived summary counts partition discovered records — each action count equals a reference count and the five terminal categories sum exactly to the discovered count. |
| `test_volume_cleanup_prerequisites_property.py` | Hypothesis property test (Property 13): prerequisite failures gate all EBS activity — discovery or deletion happens only when exact identity is established and cluster absence is verified; every other state yields a blocked outcome, no EBS action, and preserved volumes. |
| `test_stacks_volume_cleanup_helper.py` | `StackManager.cleanup_regional_volumes_after_destroy` prerequisite gates — a non-regional stack yields no outcome and no AWS call, an unresolved identity or unverified cluster absence yields a zero-discovery blocked outcome, and only a definitive deletion plus verified absence runs cleanup. |
| `test_stacks_volume_cleanup_barrier.py` | The orchestrated regional volume-cleanup barrier — where it runs relative to the regional workers, watchdog finalization, and global destruction; how it publishes one deterministic `ebs-volumes` outcome per exact regional target; and how it aggregates cleanup failure without relabeling a stack that was deleted. |
...
| `test_live_validation_volume_scenario.py` | `scripts/live_release_validation/volume_scenario.py` + `ownership/volumes.py` + `models.RunSettings` + `__main__` | The strict-validation EBS volume-scenario identity contract, fully offline: only `disabled`, `retain-override`, and `delete` are checkpoint identities while the top-level `both` selection stays a driver instruction that expands into two disjoint run IDs; the scenario case and the explicit retained-fixture cleanup authorization are part of the resume identity, so a resumed run cannot change case; the checkpoint's `volume_scenario` section is created once and pinned to one case; and the authorization gate re-verifies checkpoint identity, the account preflight recorded, the live branch, fixture-cleanup authority, and exact stack ownership before any EKS/EC2 request (incomplete stack identity blocks with no AWS call). |
| `test_live_validation_volume_scenario_driver.py` | `scripts/live_release_validation/scenario_driver.py` + `__main__` | The two-lifecycle EBS volume-scenario driver, fully offline: `--volume-scenario both` expands into two fresh runs in driver order (`<run-id>-volumes-retain-override` then `<run-id>-volumes-delete`), each with its own sibling private report directory, checkpoint, and resume identity, planned without creating any run state; every plan that would let one case observe or overwrite the other's checkpoint is refused before anything runs (shared checkpoint, shared or nested report directory, duplicated run ID or identity, settings fenced to the wrong case, settings pre-set to resume, an ignored derived run ID); execution is sequential and the first case that exits non-zero or raises stops the next lifecycle with a recorded `not-started` result; and the CLI hands the driver one base run ID plus both cases while rejecting `--resume`, `--report-dir`, and `--checkpoint`, which would pin both lifecycles together. Every lifecycle runner is injected, so no live run starts. |
| `test_live_validation_volume_inventory.py` | `scripts/live_release_validation/actions/volume_inventory.py` + `checks/volumes.py` | The strict pre-destroy PVC/PV/EBS inventory, fully offline: PVC identity, requested size, bound PV identity/CSI driver/`volumeHandle`, and normalized EBS identity/Region/AZ/size/state/attachments/exact cluster tag are recorded from live objects only, with an explicit non-participation reason for unbound, non-EBS, missing-handle, absent, out-of-scope, and unnormalizable volumes and per-PVC continuation around each; Prometheus and Alertmanager PVCs are discovered from their live component labels and their observed 50 GiB / 5 GiB sizes asserted separately; and the action is a recorded no-op without a selected scenario, authorizes stack ownership before any cluster or EC2 request, and persists each Region's evidence as it is observed. |
| `test_live_validation_volume_inventory_fencing.py` | `scripts/live_release_validation/actions/volume_inventory.py` + `ownership/volumes.py` + `runner` checkpoint I/O | The fencing, durability, and lockstep boundary of the pre-destroy volume inventory, fully offline: every strict authorization failure that reaches the action (drifted checkpoint identity, unverified or foreign account, branch drift, refused CloudFormation stack ownership) blocks its Region before any cluster or EC2 request while still leaving durable failure evidence; a volume is recorded only for the exact cluster tag key (trailing-space, case, prefix, and suffix near-misses are recorded as out-of-scope) and its tag value is recorded verbatim rather than normalized towards `owned`; each Region's evidence reaches the real owner-only atomic checkpoint (0700 directory, 0600 file, no residue) before the next Region is read and a resumed run reads it back from disk with its per-Region authorization targets; and resume refuses a changed fixture-cleanup authorization, a disabled scenario, the sibling lifecycle's identity, and any fresh run that would overwrite an existing checkpoint. Also holds the action's registry/export/runbook/README lockstep. |
| `test_live_validation_volume_target_capture.py` | `scripts/live_release_validation/ownership/volume_targets.py` + `actions/destroy.py` | Strict destroy-time capture of exact regional EBS volume-cleanup targets, fully offline: a complete capture records the exact CloudFormation stack ARN, Region, EKS cluster physical ID, exact cluster tag key, and recorded volume identities for every configured Region behind the scenario authorization gate, persisted per Region and with no EKS or EC2 client created; every way that identity can be missing or ambiguous (no checkpointed inventory, a failed inventory, a missing or drifted stack ARN, a missing or foreign cluster physical ID, a tag-key mismatch, a duplicated/out-of-Region/foreign-tagged or unobserved recorded volume, refused stack authorization) is captured as a blocked target with a machine-readable reason, persisted, and then raised, while a sibling Region's complete capture survives; only complete strict targets are reconstructed for the common cleanup helper; and the destroy action captures identity before the orchestrator deletes anything, so a blocked identity stops teardown instead of destroying the evidence. |
| `test_live_validation_volume_destroy_request.py` | `scripts/live_release_validation/ownership/volume_requests.py` + `actions/destroy.py` | The strict destroy step's command-equivalent volume policy and durable cleanup evidence, fully offline: each live case resolves through the same command-aware resolver Click uses, so `retain-override` supplies exactly `gco stacks destroy-all -y --retain-volumes` and `delete` supplies exactly `gco stacks destroy-all -y` with `delete_volumes=False` — proving the implicit-delete path needs neither the delete flag nor a volume confirmation — while a resolution that stopped meaning its case's policy, one that would need a prompt, a drifted checkpoint identity, and the `both` driver instruction all fail closed; the one resolved request reaches `destroy_orchestrated` unchanged (and a disabled scenario passes none); and teardown is marked complete only after every `ebs-volumes` callback is durable in the persisted checkpoint, with a missing, duplicated, foreign-target, identity-less, foreign-policy, or other-attempt outcome persisting blocked evidence and stopping completion. |
| `test_live_validation_volume_post_destroy.py` | `scripts/live_release_validation/checks/volume_outcomes.py` + `cleanup/volume_fixtures.py` + `actions/destroy.py` + `actions/final_inventory.py` | The independent post-destroy EBS verification, retained-fixture cleanup, and residual accounting, fully offline against an in-memory EC2 double: the retain-override case verifies that every checkpointed volume still exists with its exact recorded tag and a retention outcome (a vanished volume, a drifted tag value, or a delete report fails the run), while the delete case verifies that every recorded volume the pre-destroy inventory showed as owned/`available`/detached is absent and every ineligible volume remains with a safety outcome (a survivor, an unprovable absence, a deleted ineligible volume, or a missing per-volume record fails the run); only the exact checkpointed volume IDs are described, and a disabled scenario, a missing inventory, a drifted checkpoint identity, and zero captured targets all fail closed; retained fixtures are deleted only after that evidence is verified and durable, only for exact checkpointed identities, only with `--confirm-ebs-fixture-cleanup`, and only through production's just-in-time recheck (a reattached volume is preserved, an already-absent one is an idempotent success, the delete case cleans nothing); and final inventory counts every recorded volume so a surviving, unauthorized, or unreadable fixture fails the run while an accepted in-flight deletion does not. The destroy action's observations and fixture cleanup are proved durable before `destroyed=True`. |
| `test_live_validation_volume_lifecycle_e2e.py` | `scripts/live_release_validation/scenario_driver.py` + `actions/volume_inventory.py` + `ownership/volume_targets.py` + `ownership/volume_requests.py` + `checks/volume_outcomes.py` + `cleanup/volume_fixtures.py` + `actions/destroy.py` + `actions/final_inventory.py` | Both EBS volume-scenario lifecycles running together, fully offline: one `--volume-scenario both` driver run in which each case executes its real evidence chain — pre-destroy PVC/PV/EBS inventory, strict target capture, the command-equivalent request (`gco stacks destroy-all -y --retain-volumes` versus `gco stacks destroy-all -y` with no delete flag and no prompt), `ebs-volumes` callbacks published by the real `VolumeCleanupService`, the durable callback barrier, independent post-destroy observation, retained-fixture cleanup, and final-inventory residual accounting — so the retain case ends with proved retention and no residual bill while the delete case's eligible volumes are actually gone (its deletions happen inside the cleanup barrier, the retain case's only after verified retention evidence is durable). Also covers cross-case evidence isolation (distinct resume identities, sibling private report directories holding only their own checkpoint, and no volume ID or byte of one case's checkpoint visible to or changed by the other), a first case that fails leaving the second undeployed with no AWS request while keeping its own resumable blocked evidence, and credential-preflight fencing across the whole chain: a missing or foreign checkpointed caller identity fails the inventory, target capture, request resolution, and post-destroy observation closed without one EC2 request. Also drives ineligible-volume preservation end to end (an attached or non-`available` recorded volume is never handed to DeleteVolume and is independently observed to have remained with a safety outcome under both the delete and retain policies) and residual EBS accounting failing the run (a retain case that authorizes no fixture cleanup, and a delete case that correctly preserves an ineligible volume, both reach final inventory with the recorded volume surfaced as a residual that fails the run rather than leaving billable storage hidden). Only CloudFormation, EKS, kubectl, and git are stood in for; no live validation run is launched. |
...
| `test_mcp_extended_coverage.py` | Branch coverage across the long tail of MCP modules — `gco_mcp/iam.py` (env-unset no-op, role assumption, failure propagation, expiration fallback), `gco_mcp/resources/tasks.py` (task-id validation, the `get_task` / `_docket` / `fetch_task` accessor chain, `_coerce_to_dict` fall-throughs across dict / `model_dump` / `__dict__` / `str()` for slotted records, protocol-unavailable stub when `server` import fails), `gco_mcp/resources/docs.py` per-bucket resource handlers and metadata-header rendering, `gco_mcp/resources/cluster.py` and `k8s.py` (validation + kubectl branches), `gco_mcp/resources/iam_policies.py` and `ci.py`, `gco_mcp/tools/docs.py` (`find_docs` query / topic / no-match / `limit <= 0`), `gco_mcp/tools/images.py` plus the lazy `_get_manager`, `AuditCaptureMiddleware` ContextVar reset, and every error path in `gco_mcp/cli_runner._run_cli`. |
...
| `test_kubectl_applier.py` | Tests for the kubectl-applier Lambda (lambda/kubectl-applier-simple/handler.py), including the PriorityClass branch's behavior: cluster-scoped create, patch-on-409 convergence of the mutable fields, and loud per-resource failure when a patch is rejected (immutable `value` changes) or create fails with a non-conflict error. |
...
| `test_volume_cleanup_multi_target_property.py` | Hypothesis property test (Property 12 of the dynamic-EBS-volume cleanup spec): across generated mixes of exact regional targets, non-regional stacks, deletion states, per-target cleanup evidence, and permuted worker completion orders, the orchestrated cleanup barrier in `cli/stacks.py` applies its one resolved `destroy-all -y` authorized-delete request to every exact regional target and no non-regional stack, keeps each published `ebs-volumes` outcome's Region, cluster, volume records, reasons, and counts to that target alone, blocks a target whose stack deletion or helper call did not complete without implying an unauthorized identity, and publishes the same deterministic outcome sequence regardless of completion order. Target resolution and cluster-absence verification run for real against recording EKS/cleanup-service doubles; no AWS calls. |
