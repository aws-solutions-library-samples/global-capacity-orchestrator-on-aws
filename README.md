...
- **EKS Capabilities & GitOps hand-off** (off by default): attach the AWS-managed [Argo CD](https://argo-cd.readthedocs.io/en/stable/), [ACK](https://aws-controllers-k8s.github.io/community/) and [kro](https://kro.run/) capabilities per regional cluster from `cdk.json`, and point the hosted Argo CD at your own tenant repository through a fenced `AppProject` that can only write into the job namespaces; `gco stacks capabilities status` reports drift and `gco stacks capabilities argocd open` opens the hosted UI — see [EKS Capabilities](docs/EKS_CAPABILITIES.md)
...
│
├── gco_mcp/                             # MCP server for LLM interaction (141 tools default, up to 199 with feature flags)
