...
*A real session: `gco autopilot` launches [Claude Code](https://code.claude.com/docs/en/overview) on [Amazon Bedrock](https://docs.aws.amazon.com/bedrock/latest/userguide/what-is-bedrock.html) (GCO's default Claude Opus profile) with the [GCO MCP server](gco_mcp/README.md) + [companion MCPs](gco_mcp/README.md#recommended-companion-mcp-servers) wired in, and it answers from the project's own MCP tools ([docs](docs/AUTOPILOT.md) · [re-record](demo/record_autopilot.sh)).*
...
- **EKS Capabilities & GitOps hand-off** (off by default): attach the AWS-managed [Argo CD](https://argo-cd.readthedocs.io/en/stable/), [ACK](https://aws-controllers-k8s.github.io/docs/) and [kro](https://kro.run/) capabilities per regional cluster from `cdk.json`, and hand tenant workloads to the hosted Argo CD through a fenced `AppProject` that can only write into the job namespaces — reading a GCO-managed per-cluster CodeCommit repository you fill with `gco stacks capabilities gitops push`, or a repository of your own; `gco stacks capabilities argocd bootstrap-identity` prepares the Identity Center sign-in, `status` reports drift and `argocd open` opens the hosted UI — see [EKS Capabilities](docs/EKS_CAPABILITIES.md)
...
| Attach AWS-managed Argo CD, ACK or kro and hand workloads to GitOps | [EKS Capabilities](docs/EKS_CAPABILITIES.md) |
...
│
├── gco_mcp/                             # MCP server for LLM interaction (141 tools default, up to 201 with feature flags)
