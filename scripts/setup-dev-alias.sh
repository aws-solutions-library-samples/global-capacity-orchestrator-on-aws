...
if [ -z "$socket" ]; then
    log ""
    log "Note: $runtime has no host container socket to share, so the function omits"
    log "the socket mount. Everyday commands work as-is; for build-heavy commands like"
    log "'gco stacks deploy-all', run them on the host with $runtime as the CDK builder"
    log "(see the dev-container section of the README)."
fi
