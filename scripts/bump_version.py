...
MCP_INSTALL_TABLE_BEGIN = "<!-- BEGIN MCP INSTALL TABLE (generated by scripts/bump_version.py; edit there) -->"
...
def _mcp_server_config(version: str) -> dict:
...
def render_mcp_install_table(version: str) -> str:
    ...
    cursor_href = (
        f"https://cursor.com/en/install-mcp?name=gco&config={urllib.parse.quote(b64_encoded, safe='')}"
    )
    vscode_href = (
        f"https://insiders.vscode.dev/redirect/mcp/install?name=gco&config={url_encoded}"
    )
