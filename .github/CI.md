...
| Python packages a job pip-installs at runtime (`moto[server]`, `pyyaml`) | the pin in `pyproject.toml` — the version the test suite itself resolves | the step sources `lib_dependency_scan.sh` and reads `extract_python_pin <package> pyproject.toml` into a local, exiting with an `::error::` annotation when extraction is empty (absent, non-exact, or two groups disagreeing) rather than installing an unpinned latest | `test_workflow_pip_installs_derive_versions_pinned_in_pyproject` fails on any workflow that restates a version `pyproject.toml` already pins. This drifted for real: the moto step pinned 5.2.2 while pyproject moved to 5.2.3 and, because the install also constrains against `requirements-lock.txt`, pip refused to resolve at all. Packages pyproject does not pin (`pip` itself) are unaffected |
| Claude Code/Codex pins, per-engine default Bedrock models, companion MCP registry | `cli/autopilot.py` / `gco.bedrock` (production modules) | `.github/scripts/autopilot_ci_contract.py` derives facts and centralizes assertions for `unit:cli:autopilot`, the dev-container step, and the Claude-only boot probe | `tests/test_autopilot_ci_contract.py` holds the dual-engine contract in lockstep with production |
