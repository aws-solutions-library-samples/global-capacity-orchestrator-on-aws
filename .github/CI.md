...
- [Kind config](#kind-config)
  - [The SQS consumer's apply path](#the-sqs-consumers-apply-path)
...
│   └── feature_request.md
├── kind/
│   └── kind-calico.yaml              # Kind cluster config for the three integration:kind:* jobs
...
|------|------------|----------------|
| `workflows/unit-tests.yml` | Unit Tests | three dynamically balanced pytest shards with a stable combined-coverage gate (exact 100% line + branch floor), explicit offline accelerator catalog/NodePool/watch-list/instance-pool validation, BATS, CLI smoke, autopilot smoke (three-engine dry-run/config validation + exact pinned Claude Code, Codex, and OpenCode installs, including OpenCode's AWS-profile environment matrix and `small_model` pin), CDK synth + config matrix, lockfile freshness, fresh install, MCP install + launch smoke, workload import checks. BATS runs under `bashcov`, and a shell line-coverage floor is enforced from the resulting SimpleCov resultset (see [shell coverage gate](#shell-coverage-gate)) |
| `workflows/inference-streaming-proxy.yml` | — | Native Node.js 24 tests for the production streaming Lambda, with exact 100% line/function/branch thresholds; renders the lcov tracefile into the HTML report `pages.yml` serves at `/nodejs-coverage/` |
...
| `workflows/integration-tests.yml` | Integration Tests | Autopilot boot probes for all three engines (`gco autopilot` on a bare runner self-installs the pinned Claude Code — and, in the codex and opencode twins, the pinned Codex or OpenCode — pre-warms and boots the in-tree gco MCP server plus every curated companion under the engine, and dispatches to Bedrock with the shipped per-engine default model, stopped fail-closed at the credential boundary by fabricated keys — AWS rejecting the signed request is the success condition: 403 under claude, SigV4's 401 under codex, whose probe also asserts the engine delta that codex races MCP init against the first turn instead of blocking on connections, and the SigV4 `security token … invalid` rejection under opencode, whose probe proves per-server connections through `opencode mcp list` because that engine logs MCP servers only when they fail, and asserts the `small_model` pin and that no pinned AWS profile bypassed the exported credentials), per-Dockerfile build + functional container tests (import plus a uvloop + httptools fast-path assertion for the four uvicorn images, boot under pod-equivalent constraints, probe/auth-fail-closed/degraded-503 HTTP contracts, kubelet exec-command shapes, SIGTERM shutdown, moto-SQS consume/reject exit codes for the queue processor), dev-container smoke (pinned toolchain incl. uv/uvx, native-arch binaries, all three engine plans/configs, and Codex's persisted lazy install across disposable containers), kind E2E with Calico and pinned Metrics Server (health-monitor and manifest-processor rollouts to Ready under the shipped NetworkPolicies — plus one CI-only egress allow for kind's `:6443` API server endpoint, which on EKS is `:443` and already admitted — the gated manifest-processor HPA pair, inference-proxy HPA `ScalingActive`, PDB shape `maxUnavailable: 1`, RBAC verification including the kro capability's tenant RBAC fence under impersonation, ResourceQuota/LimitRange, NetworkPolicy enforcement probed both ways — cross-namespace denied, same-namespace and the inference-monitor metrics port admitted — and a server-side `--dry-run` of the inference Deployments the in-cluster monitor renders through its production `_build_inference_deployment_object` for the vLLM default, the SGLang default with its injected launcher, and SGLang with operator-supplied launcher flags and placement, so the apiserver's admission chain sees the rendered objects before a live deploy does), kind cost pipeline (the real `cost-monitor:ci` image against the pinned OpenCost and kube-prometheus-stack charts from `charts.yaml`, with the digest-pinned Floci emulator as the report bucket — Prometheus proven to scrape OpenCost, `/internal/status` reaching `opencost_healthy` + `opencost_returning_data` through the shipped `allow-cost-monitor-to-opencost` egress rule under Calico, an ad-hoc `POST /internal/reports` whose Parquet object is read back inside the monitor's own container with its columns checked against `ALLOCATION_REPORT_FIELDS`, and a label-only egress probe proving an unlabelled gco-system pod is denied OpenCost), kind examples smoke (Calico-enforced, pinned kubeflow-trainer + mlflow charts installed with the exact `charts.yaml` values via `validate_helm_charts.py --emit-ref/--emit-values`, ServiceMonitor CRD from the pinned kube-prometheus-stack, the post-Helm mlflow NetworkPolicies applied with the deployment token filled from kind's own node CIDR so the kubelet-probe allow is exercised for real — the chart's policy admits only pod sources and silently drops probes, which is invisible under kindnet, the real `examples/kubeflow-trainjob.yaml` applied as the manifest-processor ServiceAccount and run to `Complete` with the all-reduce sentinel verified, a `SubjectAccessReview` (SAR) sweep derived from the submission allowlist — SAR is the `authorization.k8s.io` object that answers "may this user *verb* this resource?", posted as an explicit body rather than through `kubectl auth can-i`, because can-i resolves its resource argument via discovery and answers "no" for a CRD kind whose chart is not installed yet, and mlflow host-validation probes — allowed Host 200 / arbitrary Host 403 / `/health` exempt), kind platform add-ons (the pinned argo-cd, crossplane and crossview charts with the `charts.yaml` values plus the stack-derived Argo CD values — a repo-server HPA that owns its replica count —, a real Git sync of `examples/gitops/hello-job` at the pull request's own commit, the `gco-tenants` project and RBAC fences under impersonation, the example Application's resources-finalizer cascade through the installer's pre-uninstall cleanup, a Crossplane v2 composition round trip with its namespaced write fence, and both dashboards captured by the CLI's screenshot code into the `platform-addon-dashboards` artifact), K8s manifest schema validation (kubeconform), Lambda import validation, cross-module pytest, MCP server pytest |
| `workflows/security.yml` | Security | bandit (preceded by the tracked demo GIF and SVG asset content validators), pip-audit, npm audit across every owned package graph, bundler-audit on the committed gem graph, trivy (filesystem + per-image matrix), trufflehog, gitleaks, semgrep, checkov, KICS, CodeQL (Python + JavaScript) |
| `workflows/lint.yml` | Linting | actionlint, action SHA-pin verification (including each version comment resolved against GitHub), hadolint, markdownlint, strict Zensical wiki build (`scripts/build_wiki.py build` — the same build `pages.yml` runs at deploy time, so wiki breakage fails pre-merge), mypy (strict / stacks / lambda), ruff (format + check, imports included), strict ShellCheck at `style` severity over every tracked `*.sh` path (NUL-safe, external sources enabled, empty inventory fails), yamllint |
...
| `workflows/pages.yml` | `workflow_run` after **Unit Tests** completes on `main` | Publish the project site to GitHub Pages via `actions/deploy-pages`: build the Zensical orientation wiki (`wiki/` + `zensical.toml`, strict mode, via `scripts/build_wiki.py build`, which first stages the pages, `images/` and the generated API spec sheets into `build/wiki`) from the triggering run's commit at the site root, with the spec sheets at `/api/`; build FastAPI's Swagger UI console per service at `/swagger/` from `docs/openapi/*.json` with the locked `swagger-ui-dist` self-hosted (`diagrams/api_specs/generate.py --swagger-ui-dir`, zero third-party requests); download that run's `pytest-coverage` and `bash-coverage-report` artifacts and the `node-inference-streaming-proxy-coverage` artifact from the **Inference Streaming Proxy** run for the same commit; serve the three reports at `/python-coverage/`, `/bash-coverage/` and `/nodejs-coverage/` (with a redirect at the old `/coverage/`); and render the three shields.io badge JSONs at the site root with `scripts/render_coverage_badges.py` (so the README badges' percent-encoded URLs never change). Split out of `unit-tests.yml` so a GitHub Pages backend stall — or a wiki build failure — surfaces here instead of failing the test gate; the PR-side `lint:zensical:strict` job runs the identical build pre-merge |
...
| `gate:lint` | `workflows/lint.yml` | every linter, type checker and the strict Zensical wiki build |
...
- `timeout-minutes` on every job, sized to the job: 2 min for path-filter and list jobs, 5 for the `gate:*` aggregators, 10 for lint, 15–20 for unit (30 for `unit:bats:shell`, which runs the shell suite twice — plain, then traced under `bashcov` at roughly twice the cost), 20–30 for integration and image builds, 45 for the Floci release-validation e2e and 60 for the Kind examples smoke run. The rule of thumb is about twice a job's current worst case: enough that a slow runner never fails a healthy job, tight enough that a hung one is killed promptly.
...
| Floci emulator image (tag + digest) | one `floci/floci:<tag>@sha256:<digest>` value, shared by every workflow that runs it | `services.floci.image` in `floci-tests.yml` (emulator-backed test layer) and `integration-tests.yml` (the queue-processor job's SQS endpoint), plus that workflow's `CI_SQS_EMULATOR_IMAGE` (cluster-e2e) and `CI_S3_EMULATOR_IMAGE` (cost-pipeline) job envs, which run the same emulator *inside* kind — as the consumer's SQS queue and as the cost monitor's report bucket respectively. GitHub does not expand `env` inside `services.*.image`, so the string is repeated and reconciled by test instead | `tests/test_pinned_floci_version.py` discovers every workflow referencing the image rather than naming them, requires all pins identical in both tag and digest, rejects any unpinned or off-pin reference, and holds `docs/FLOCI_TESTING.md`'s local-run tag to the same value |
| Claude Code/Codex/OpenCode pins, per-engine default Bedrock models, companion MCP registry | `cli/autopilot.py` / `gco.bedrock` (production modules) | `.github/scripts/autopilot_ci_contract.py` derives facts and centralizes assertions for `unit:cli:autopilot`, the dev-container step, and all three engines' boot probes | `tests/test_autopilot_ci_contract.py` holds the three-engine contract in lockstep with production |
...
The README's badges sit on four rows. The first carries the four workflow
health signals, which change with every run; the second carries the three
coverage numbers, one per test stack; the third carries the two stable
pointers, so a reader scanning for "where do I go next" is not parsing build
status; the fourth carries the license, which is legal metadata rather than a
place to go next. The five parts, in order:

1. **Four workflow-status badges** (`Unit Tests`, `Integration Tests`, `Security`, `Linting`) from GitHub's native `badge.svg` endpoint.
2. **Three coverage badges** (Python, Bash, Node.js) rendered by shields.io from the endpoint JSON that `pages.yml` publishes at the Pages site root (`/python-coverage-badge.json`, `/bash-coverage-badge.json`, `/nodejs-coverage-badge.json`), each generated by `scripts/render_coverage_badges.py` from the same run whose HTML report is served at `/python-coverage/`, `/bash-coverage/` or `/nodejs-coverage/` — each badge links to its report. For every stack, badge, report and the exact 100% gate all describe one run: coverage.py's `totals.percent_covered` (statements and branches), the shell checker's statement summary after its lexer corrections, and Node's lcov tracefile (lines and branches). Bright green at exactly 100%, red below it, so the badges add no second threshold.
...
4. **A wiki badge** — a static shields.io badge linking to the Pages site root, where `pages.yml` serves the Zensical-built wiki. It sits last on its row so the dynamic quality signals stay grouped.
5. **A license badge** — a static shields.io badge (`license: MIT-0`) on its own row, linking to the repository's `LICENSE` file by relative path. It is static rather than the API-backed `img.shields.io/github/license/<owner>/<repo>` form so it carries no owner/repo path for `scripts/migrate_fork.py` to rewrite and does not depend on GitHub's license detection on a fork; it only changes if the license does.

### "repo or workflow not found" on fresh or private repositories

The four workflow-status badges at the top of the README come from GitHub's native `badge.svg` endpoint and render a placeholder image when the repo is unreachable. The wiki and license badges (`img.shields.io/badge/...`) are static and always render; the three coverage badges (`img.shields.io/endpoint`) render whatever badge JSON the Pages site last published, so they need one successful `pages.yml` deploy before they show a number.
...
| GCO Autopilot pins | `CLAUDE_CODE_VERSION`, `CODEX_VERSION`, `OPENCODE_VERSION`, and `COMPANION_MCP_SERVERS` in `cli/autopilot.py` (via `extract_claude_code_pin` / `extract_codex_pin` / `extract_opencode_pin` / `extract_companion_mcp_packages`) | Public endpoints, no AWS creds. The Claude Code, Codex, and OpenCode install pins are each compared against the npm `latest` dist-tag; each companion MCP server is resolved on its registry (`get_registry_package_status`) and reported when missing, deprecated, or yanked — companions are launched unpinned by npx/uvx, so registry health *is* the dependency surface. All these constants live in Python, invisible to Dependabot |
| Pre-commit hooks | `repo:` / `rev:` blocks in `.pre-commit-config.yaml` | Calls `GET /repos/{owner}/{repo}/tags` on GitHub for each hook and reports drift when our pinned `rev:` is older than the highest semver-shaped tag. Authenticated with the job token like every other `api.github.com` read (see [GitHub API lookups](#github-api-lookups)); full 40- or 64-character Git object IDs are accepted as immutable exemptions, while unsupported revisions and failed lookups make the scan incomplete |
...
| Ruby release | `.ruby-version` (the interpreter series the shell-coverage gate runs on) | Queries `endoflife.date/api/ruby.json` for the newest shipped, still-supported series. Dependabot watches the gems in `Gemfile.lock` but says nothing about the interpreter, so without this the pin could sit on an end-of-life series indefinitely. Informational — a new Ruby series is a deliberate move, not a security patch |
| Runner images | `runs-on:` values across `.github/workflows/*.yml`, plus the `runner:` matrix values an expression-valued `runs-on:` resolves to | Public endpoint, no AWS creds. Compares each label against the `Available Images` table in [`actions/runner-images`](https://github.com/actions/runner-images#available-images) via `check_runner_images.py`. `runs-on: ubuntu-latest` is not a version pin, so Dependabot has nothing to bump. A newer **generally-available** image — or a label upstream marked **deprecated**, which is a removal notice with a date — counts as drift; a newer image still in **preview** is reported as a note instead, because moving to a preview trades a stable CI platform for an unannounced one. Never compares across image families or architectures |
| CI tooling | Trivy (the `install-trivy` action's `version` default), `ACTIONLINT_VERSION` (`lint.yml`), Helm and kubectl (the `lambda/helm-installer/Dockerfile` RUN-line pins every workflow derives from), `KUBECONFORM_VERSION`, `CALICO_VERSION`, and `METRICS_SERVER_VERSION` (`integration-tests.yml`), the kind binary + node image (the workflow-level env of `integration-tests.yml`), and the Crossplane `function-go-templating` package tag in `post-helm-crossplane.yaml` | Public endpoints, no AWS creds. Compares each hand-installed CI tool against its upstream (GitHub Releases for Trivy/actionlint/Helm/kind/kubeconform/Calico/Metrics Server/function-go-templating — authenticated with the job token, see [GitHub API lookups](#github-api-lookups) — `dl.k8s.io` for kubectl, registry tags within the pinned minor for the kind node image). These are plain env / `with:` pins Dependabot doesn't watch — stale linting, validation, networking, and autoscaling tools can silently weaken CI coverage, and the composition function Crossplane runs would age unnoticed (an xpkg reference, not an `image:` line the image sweeps read; the add-on charts and Crossview's image are `charts.yaml` pins the Helm sections cover) |
...
Fifteen of the scan's upstream reads go to `api.github.com`: the seven CI-tooling
releases, one tags page per GitHub-hosted pre-commit hook, and the four
Dockerfile.dev pins that live in GitHub repositories (`AWSCLI_VERSION`,
`DOCKER_VERSION`, `BUILDX_VERSION`, `UV_VERSION`). All of them go through one
helper, `github_api_get` in `lib_dependency_scan.sh`, and the workflow passes
the job's own `GITHUB_TOKEN` to the scan step so they are sent as bearer-token
requests. That is not a permissions matter — every repository queried is
public and `contents: read` is enough — it is a rate-limit one. Anonymous
requests draw from a 60 req/h bucket keyed on the client IP, and on a
GitHub-hosted runner that IP is shared with every other tenant's job on the
same host, so a run can start with the bucket already empty. When that
happens every GitHub lookup fails at once while PyPI, npm, the OCI registries
and the Helm repos (different hosts, different limits) all succeed, and the
monthly report arrives with fifteen "lookup failed" entries and every
GitHub-backed surface marked provisional. The per-token bucket is 5 000 req/h
and is not shared. A refused token (an organisation that blocks the GitHub
Actions app answers 401/403 to a workflow token on its *public* repositories;
aquasecurity does) falls back to an anonymous read of the same URL, the same
fallback `verify_action_pins.py` carries.

The helper also records *why* a call failed, and the incomplete-lookup line
carries it: `(GitHub API: rate limit exceeded on the anonymous 60 req/h bucket,
set GITHUB_TOKEN)`, `(GitHub API: rate limit exceeded for the authenticated
token)`, `(GitHub API: HTTP 404)`, or `(GitHub API: transport error)`. A
403 counts as a rate limit only when the quota headers say so
(`x-ratelimit-remaining: 0`, or a `retry-after` from a secondary limit); a
403 with quota left is reported as what it is. Locally, `gco deps scan` sends
the developer's `GITHUB_TOKEN` or `GH_TOKEN` when either is exported and stays
anonymous otherwise.
...
| `GITHUB_TOKEN` / `GH_TOKEN` | unset | Bearer token for the `api.github.com` release and tag lookups (see [GitHub API lookups](#github-api-lookups)). The workflow passes the job token; `gco deps scan` forwards whichever a developer has exported. Unset, those lookups run anonymously against the shared 60 req/h bucket. |
...
| Every GitHub-backed lookup incomplete at once, other registries fine | The `api.github.com` calls ran anonymously and the runner's shared 60 req/h bucket was empty; the incomplete-lookup lines say `rate limit exceeded on the anonymous 60 req/h bucket, set GITHUB_TOKEN`. In the workflow the scan step passes `GITHUB_TOKEN: ${{ github.token }}`; locally export `GH_TOKEN` (see [GitHub API lookups](#github-api-lookups)) |
...
SimpleCov's own HTML lands in `coverage/index.html`; it renders the raw line hits, so it shows the two corrected shapes as misses and reads lower than the gate. The report to read is the checker's: `--report coverage/report` (which CI passes) writes a statement-level `index.html`, one page per script and a `summary.json` from the same corrected data the verdict comes from, and `pages.yml` serves that at `/bash-coverage/` and renders the README's Bash badge from the summary. CI uploads the whole `coverage/` directory as the `bash-coverage-report` artifact. Measure on Linux: bashcov reads the whole trace through one pipe, and macOS's 512-byte `PIPE_BUF` lets concurrently tracing shells tear each other's records, which aborts the parser. The Ubuntu job is authoritative; a container with the job's apt packages (`bash`, `bats`, `jq`, `python3` with PyYAML, `nodejs`) and the pinned Ruby reproduces it exactly.
...
- **`kind/kind-calico.yaml`** — kind cluster config with `disableDefaultCNI: true` so Calico can be installed on top and actually enforce the `NetworkPolicy` resources from `lambda/kubectl-applier-simple/manifests/03-network-policies.yaml`. The default kindnet CNI does not enforce NetworkPolicy. Used by the three Calico kind jobs (`integration:kind:cluster-e2e`, `integration:kind:cost-pipeline`, `integration:kind:examples-smoke`); `integration:kind:platform-addons` keeps kind's default CNI because nothing it proves depends on NetworkPolicy enforcement.

### The SQS consumer's apply path

`integration:kind:cluster-e2e` is also where the SQS submission loop closes, because it is the only job with both halves the queue processor needs: a real API server and an SQS wire API. `integration:docker:queue-processor` runs the same image against the emulator with a *stub* kubeconfig, which covers the empty-queue exit, the policy rejection that must retain its message, and the missing-configuration exit — every path that stops before the apply.

The kind job supplies the rest, deliberately reusing production artifacts rather than restating them:

- The **message** is built by `gco.job_envelope.build_job_message`, the same function `gco jobs submit-sqs` calls, from the CPU Job in `examples/sqs-job-submission.yaml` — the manifest the docs tell users to submit. Selected by name, so reordering the example cannot switch the test to the GPU job.
- The **consumer** is the shipped `post-helm-sqs-consumer.yaml` ScaledJob's `jobTargetRef`, lifted out and applied as the `batch/v1` Job KEDA would have created (KEDA itself arrives through Helm, which this raw-manifest job does not install). The image, the `gco-manifest-processor-sa` ServiceAccount, the two projected service-account tokens, the non-root read-only security context, and the whole validation-policy env block are production's. Only three things change: static credentials plus `AWS_ENDPOINT_URL_SQS` replace the IRSA variables that resolve only on EKS, and `JOBS_TABLE_NAME` is empty because the failure-record path has no DynamoDB here.
- The **queue** is the digest-pinned Floci emulator, this time as a Deployment in its own `gco-ci-sqs` namespace. gco-system's shipped default-deny egress is part of what the job tests, so instead of relaxing it, one CI-only NetworkPolicy admits exactly `app: queue-processor` → that namespace on 4566. The queue is created and the message sent through a `kubectl port-forward`; the URL the emulator returns names the forwarded loopback, so its authority is rewritten to the in-cluster Service before the consumer sees it.

The assertions are the ones that would catch a real break: the consumer Job reaches `Complete` (both terminal conditions are polled, so a failed Job fails fast with its logs), the submitted manifest exists in `gco-jobs` with the example's own labels and image intact — proving the manifest-processor SA's RBAC really permits the create — and the queue drains to zero, which is the consumer's only success signal.

### The kro capability's tenant RBAC

The same job is where `07-kro-tenant-access.yaml`, the one base-pass file the opt-in [kro EKS Capability](../docs/EKS_CAPABILITIES.md) switches on, meets a real API server. kro itself is AWS-managed and has no kind equivalent (the example harness covers it live), but its RBAC is plain Kubernetes: the job renders the file with the helpers the regional stack uses (`gco.eks_capabilities_config.kro_kubernetes_username` for the session user the capability acts as, `compute_eks_capabilities_replacements` for the token), applies it, and proves the fence by impersonating that user. The allow-listed kinds are readable cluster-wide but Secrets are not; writes land in `gco-jobs`/`gco-inference` only; the guardrail kinds (`NetworkPolicy`, `ResourceQuota`, `LimitRange`, `Role`, `RoleBinding`) are never writable; nothing in `gco-system`, `kube-system` or at cluster scope is. The applier's own disable-path inventory (`_FEATURE_RESOURCE_INVENTORY`) then has to remove exactly what the file applied.

### The platform add-ons against the real charts

`integration:kind:platform-addons` is where the self-managed [Argo CD](../docs/GITOPS.md) and [Crossplane](../docs/CROSSPLANE.md) add-ons meet a real API server before a live deploy. It reuses the production artifacts rather than restating them: the argo-cd, crossplane and crossview charts at their `charts.yaml` versions, namespaces and values (`validate_helm_charts.py --emit-ref/--emit-values`, prefetched with bounded retries and installed once from the local archive, in the installer Lambda's order), the chart values the stack derives from `helm.argocd` (`gco.argocd_config.argocd_chart_values`, merged over the shipped values the way the installer's `deep_merge` does), and the post-Helm manifests rendered by `compute_argocd_replacements`. kind's default CNI is enough: nothing here depends on NetworkPolicy enforcement.

- **Argo CD.** The block turns the repo-server autoscaler on with a floor of two. The job checks the chart's HPA (the repo-server container's CPU, the damped behavior), that the rendered Deployment leaves `replicas` out, and that the HPA lifts it to two ready pods; no metrics-server is needed for that, because the HPA clamps to `minReplicas` before it reads a metric. The root Application then syncs `examples/gitops/hello-job` from GitHub at the pull request's own head commit (fork PRs included) and the Job completes. A probe Application aimed at `gco-system` is refused by the `gco-tenants` project, and `kubectl auth can-i` impersonation proves the RBAC fence: the controller writes the tenant allow-list in the two tenant namespaces and nothing else, and the server only reads. Deleting the root Application detaches the Job; the shipped `examples/argocd-gitops-job.yaml`, repointed at this commit, adopts it; and the helm installer's own `_delete_chart_custom_resources` cascades its resources finalizer before the applier's disable-path inventories and `helm uninstall` remove the rest (the CRDs stay). The cascade has to finish inside the installer's wait, with the AppProjects in a later pass: a run that needs the finalizer-strip fallback fails.
- **Crossplane.** The go-templating Function turns Healthy, `examples/crossplane-batch-api.yaml` serves `BatchJob`, `examples/crossplane-batch-job.yaml` composes a Job in `gco-jobs` that completes and marks the XR Ready, and an XR in `gco-system` is refused by the namespaced write grant. The Crossplane and Crossview fences hold under impersonation, Crossview answers `/api/health`, and teardown runs the installer cleanup (the packages in their own last pass, again without the finalizer-strip fallback) and the applier prune.
- **Dashboards.** Both UIs are captured by the code `gco gitops screenshot` and `gco crossplane screenshot` run once their tunnel is up (port-forward, the Argo CD API login and session cookie, headless Chromium from the pinned Playwright) and uploaded as the `platform-addon-dashboards` artifact, the source of `images/argocd-ui.png` and `images/crossview-dashboard.png`.

`test_kind_platform_addons_is_a_real_artifact_test_that_stays_fail_fast` in `tests/test_supply_chain_integrity.py` pins that shape.

### The cost pipeline against the real OpenCost

`integration:kind:cost-pipeline` is the one place the three processes of the cost pipeline meet before a live deploy: Prometheus scrapes the cluster and OpenCost, OpenCost prices allocations from those series, and the cost monitor turns OpenCost's answer into Parquet in S3. Every other layer checks one of them in isolation — the unit suite feeds the monitor a hand-written `/allocation/compute` body, the Floci suite proves the S3 write against an OpenCost double, `integration:docker:cost-monitor` boots the image against an unreachable OpenCost, and `integration:kind:cluster-e2e` scales the monitor to zero before its rollouts (no AWS, and the node's CPU goes to the API tiers). The seams that only break for real — the response shape of the *pinned* OpenCost release, the shipped `charts.yaml` Prometheus wiring and ServiceMonitor, the `allow-cost-monitor-to-opencost` egress rule under an enforcing CNI, and `opencost_returning_data`, the status flag `gco release validate` gates on — used to be exercised for the first time on a real cluster.

The job reuses production artifacts rather than restating them. The charts are the installer Lambda's — `kube-prometheus-stack` and `opencost` at the `charts.yaml` version, namespace and static values via `validate_helm_charts.py --emit-ref/--emit-values`, prefetched once with bounded retries and installed from the local archive — and the only overlay is the deploy-time layer `GCORegionalStack._observability_chart_values` / `_opencost_chart_values` inject (the `cdk.json` retention and sizes, the `gco-observability-gp3` class name aliased to kind's local-path provisioner, the cluster id) plus CI-only omissions that sit outside the path under test and are named where they are set: Grafana, Alertmanager, node-exporter's host root mount, and the OpenCost UI. The monitor is `cost-monitor:ci` from the shipped Dockerfile, rendered from the shipped `34-cost-monitor.yaml` with the production `OPENCOST_BASE_URL` and probes untouched; as with the queue processor, the two IRSA variables go and static emulator credentials come in, with `AWS_ENDPOINT_URL` set by ClusterIP because botocore addresses a hostname endpoint virtual-hosted-style. gco-system's shipped policies are applied so the egress rule is a real verdict; one CI-only rule admits `app: cost-monitor` → the emulator namespace on 4566 and nothing else.

The assertions follow the data: Prometheus reports an `up` OpenCost target and a `node_total_hourly_cost` series (the failure the monitor's data probe exists to catch, asserted at its source); `/internal/status` reaches `opencost_healthy` and `opencost_returning_data` within a bounded wait; `POST /internal/reports` returns a key under `adhoc/region=us-east-1/`, which the monitor's own `GET` lists, `aws s3api head-object` finds in the emulator, and the monitor's container reads back with the boto3 + pyarrow it wrote with — columns exactly `ALLOCATION_REPORT_FIELDS` (the Glue table contract in `gco/stacks/monitoring_stack.py`), matching row count, every row stamped with the injected cluster id, the `monitoring` namespace present, at least one row priced; and two identical busybox probes differing only in label prove the OpenCost rule is label-scoped, not namespace-wide. `test_kind_cost_pipeline_runs_the_real_monitor_against_the_pinned_charts` in `tests/test_supply_chain_integrity.py` pins that shape.
