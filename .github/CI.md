...
|------|------------|----------------|
| `workflows/unit-tests.yml` | Unit Tests | pytest with coverage (90% enforced floor; ~92% Python target), explicit offline accelerator catalog/NodePool/watch-list/instance-pool validation, BATS, CLI smoke, autopilot smoke (`gco autopilot` dry-run/config validation + pinned Claude Code install), CDK synth + config matrix, lockfile freshness, fresh install, MCP install + launch smoke, workload import checks |
...
| Bedrock default models | `context.bedrock.default_model_id` (advisory: Mission sampling + capacity advisor), `context.bedrock.claude_code_default_model_id` (`gco autopilot`'s Claude Code session model), and `context.bedrock.embedding_model_id` (Mission memory's text-embedding model) from `cdk.json`, each resolved through `gco.bedrock` | Requires AWS credentials (via OIDC). The generation keys compare against system-defined inference profiles (`bedrock list-inference-profiles`); the embedding key compares against `bedrock list-foundation-models --by-output-modality EMBEDDING` (both pinned to us-east-1). Drift is reported per key only within the *same model family*. Dependabot does not inspect these deployment configuration values |
...
| GCO Autopilot pins | `CLAUDE_CODE_VERSION` and `COMPANION_MCP_SERVERS` in `cli/autopilot.py` (via `extract_claude_code_pin` / `extract_companion_mcp_packages`) | Public endpoints, no AWS creds. The Claude Code install pin is compared against the npm `latest` dist-tag; each companion MCP server is resolved on its registry (`get_registry_package_status`) and reported when missing, deprecated, or yanked — companions are launched unpinned by npx/uvx, so registry health *is* the dependency surface. Both constants live in Python, invisible to Dependabot |
...
- **New default Bedrock model** — change `cdk.json` `context.bedrock.default_model_id` (advisory: `gco.bedrock` supplies it to Mission and the capacity advisor) and/or `context.bedrock.claude_code_default_model_id` (the independent default `gco autopilot` hands to Claude Code), while `tests/test_default_bedrock_model_consistency.py` guards the runtime aliases, all pins, and packaged config. The "Checking Bedrock default model" section tracks each key's model family automatically, including `context.bedrock.embedding_model_id` (Mission memory). Embedding drift carries a data caveat: stored vectors are only comparable to vectors from the same model, so re-embed or segregate existing Mission-memory data when adopting a newer embedding model. If a new advisory model has no captured scaffold fixture yet, run `python scripts/capture_scaffold_fixtures.py --model <id>`.
