...
| Claude Code pin, default Bedrock model, companion MCP registry | `cli/autopilot.py` / `gco.bedrock` (production modules) | `.github/scripts/autopilot_ci_contract.py` derives facts and centralizes the assertions for `unit:cli:autopilot`, the dev-container step, and the boot probe | `tests/test_autopilot_ci_contract.py` holds the contract in lockstep with the production registries |
