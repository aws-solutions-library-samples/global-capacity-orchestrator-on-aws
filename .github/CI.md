...
| CI tooling | Trivy (the `install-trivy` action's `version` default), `ACTIONLINT_VERSION` (`lint.yml`), Helm and kubectl (the `lambda/helm-installer/Dockerfile` RUN-line pins every workflow derives from), `KUBECONFORM_VERSION`, `CALICO_VERSION`, `METRICS_SERVER_VERSION`, and `ARGOCD_VERSION` (`integration-tests.yml`), and the kind binary + node image (the workflow-level env of `integration-tests.yml`) | Public endpoints, no AWS creds. Compares each hand-installed CI tool against its upstream (GitHub Releases for Trivy/actionlint/Helm/kind/kubeconform/Calico/Metrics Server/Argo CD — authenticated with the job token, see [GitHub API lookups](#github-api-lookups) — `dl.k8s.io` for kubectl, registry tags within the pinned minor for the kind node image). These are plain env / `with:` pins Dependabot doesn't watch — stale linting, validation, networking, and autoscaling tools can silently weaken CI coverage, and the Argo CD CRDs the kind job validates the capability manifests against would silently fall behind the schema the hosted capability installs |
...
- **`kind/kind-calico.yaml`** — kind cluster config with `disableDefaultCNI: true` so Calico can be installed on top and actually enforce the `NetworkPolicy` resources from `lambda/kubectl-applier-simple/manifests/03-network-policies.yaml`. The default kindnet CNI does not enforce NetworkPolicy. Used by all three kind jobs (`integration:kind:cluster-e2e`, `integration:kind:cost-pipeline`, `integration:kind:examples-smoke`).
...
### The Argo CD capability manifests against the real argoproj.io CRDs

The same job is where the two manifests behind the opt-in [Argo CD EKS Capability](../docs/EKS_CAPABILITIES.md) (`07-argocd-cluster-access.yaml`, `08-argocd-gitops.yaml`) meet a real API server. In production the AWS-managed capability installs the `Application`/`AppProject` CRDs before the applier runs; the job installs the same CRDs from the pinned upstream Argo CD release (`ARGOCD_VERSION`, a job-level env pin the [monthly dependency scan](#dependency-scan-script) compares against the latest `argoproj/argo-cd` release; each of the two CRD manifests is downloaded to a file and checked against its committed `ARGOCD_*_CRD_SHA256` before a server-side apply — server-side because the Application schema overflows the last-applied annotation), renders both files with the pure renderer the regional stack uses (`gco.eks_capabilities_config.compute_eks_capabilities_replacements` — cluster ARN, capability role ARN, JSON-rendered destinations and sync policy) from a validated `source: git` block pointing at this repository's `examples/gitops/tenant-smoke` (the CodeCommit default would name a repository that exists only after a regional stack deploys; the live harness covers that source), and applies them. The apiserver validates the fenced `AppProject` and root `Application` against the real schemas, and the RBAC fence is proved by impersonating the access-entry group the capability role maps to: cluster-wide read and workload writes in `gco-jobs`/`gco-inference` are allowed; writes in `gco-system`/`kube-system`, cluster-scoped creates, and — inside the tenant namespaces themselves — the guardrail kinds the `AppProject` blacklists (`NetworkPolicy`, `ResourceQuota`, `LimitRange`, `Role`, `RoleBinding`) are denied, so the Kubernetes allow-list and the Argo CD fence are proved to agree. A second step runs the applier's own prune inventories (`_FEATURE_RESOURCE_INVENTORY`, the disable path) against the live cluster and requires every inventoried object gone while the `argocd` Namespace survives by design.
