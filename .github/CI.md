...
| Bedrock default models | `context.bedrock.default_model_id` (advisory: Mission sampling + capacity advisor) and `context.bedrock.claude_code_default_model_id` (`gco autopilot`'s Claude Code session model) from `cdk.json`, each resolved through `gco.bedrock` | Requires AWS credentials (via OIDC). Lists system-defined inference profiles (`bedrock list-inference-profiles`, pinned to us-east-1) and reports drift per key when a newer release exists in the *same model family*. Dependabot does not inspect these deployment configuration values |
...
- **New default Bedrock model** — change `cdk.json` `context.bedrock.default_model_id` (advisory: `gco.bedrock` supplies it to Mission and the capacity advisor) and/or `context.bedrock.claude_code_default_model_id` (the independent default `gco autopilot` hands to Claude Code), while `tests/test_default_bedrock_model_consistency.py` guards the runtime aliases, both pins, and packaged config. The "Checking Bedrock default model" section tracks each key's model family automatically. If a new advisory model has no captured scaffold fixture yet, run `python scripts/capture_scaffold_fixtures.py --model <id>`.
