...
| `check_bash_coverage.py` | `unit-tests.yml` (`unit:bats:shell`) | Enforces the shell-script line-coverage floor from the bashcov/SimpleCov resultset the BATS suite produces. Deciding which lines of a shell script are executable (here-documents, `case` arms, line continuations, function headers) is left to SimpleCov rather than re-implemented; what this owns is everything SimpleCov cannot know about this repository. Reported paths are absolute, so each is mapped back onto the tracked script it refers to by longest path suffix — falling back to a unique basename — and hits from every path that maps to the same script are merged, so one script exercised by several suites is credited with all of them instead of appearing as two half-covered files. Scripts not yet fully covered are listed in `[tool.bash-coverage] ratchet` in `pyproject.toml`; everything else must be at 100%. The gate fails closed: a tracked script absent from the report entirely is an error, not a pass, because that is what a mis-scoped bashcov run or a suite that never executes its subject looks like. Unmeasured scripts are rendered as `n/a` rather than a percentage, so nobody strikes an untested script off the ratchet by misreading `100%`. Exit `2` is reserved for a missing or unparseable report so "never ran" stays distinguishable from "not covered". Importable (`evaluate()`, `parse_report()`, `map_to_tracked()`, `load_ratchet()`, `main()`) — `tests/test_check_bash_coverage.py` exercises the whole decision surface without Ruby or bats, and holds the ratchet's entries to real tracked scripts. |
