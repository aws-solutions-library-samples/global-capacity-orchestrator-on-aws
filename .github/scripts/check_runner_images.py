...
def fetch_readme(url: str = README_URL, timeout: int = FETCH_TIMEOUT_SECONDS) -> str:
    ...
    """
    try:
        with urllib.request.urlopen(  # nosec B310  # nosemgrep: dynamic-urllib-use-detected - the default is the fixed https://raw.githubusercontent.com literal in README_URL and no caller in this repository overrides it; the parameter exists only so the tests can inject a stub, so no scheme or host is ever derived from repository content
            url, timeout=timeout
        ) as response:
