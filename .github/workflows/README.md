...
| `inference-streaming-proxy.yml` | — | Native Node.js 24 tests for the inference streaming [Lambda](https://docs.aws.amazon.com/lambda/latest/dg/welcome.html) with exact 100% line/function/branch gates; renders the lcov tracefile into the HTML report `pages.yml` serves at `/nodejs-coverage/` |
...
| `integration-tests.yml` | Integration Tests | Dockerfile builds + functional container tests (uvloop + httptools fast-path assertion for the uvicorn images, pod-equivalent boot, HTTP probe/auth contracts, SIGTERM shutdown, moto-SQS queue-processor exit codes), dev-container smoke (pinned toolchain incl. uv/uvx + all three engine plans/configs + persisted Codex lazy install), kind cluster E2E with Calico and Metrics Server (5 service deployments, inference-proxy HPA `ScalingActive`, RBAC enforcement, NetworkPolicy blocking, ResourceQuota, PDB validation, server-side dry-run of the rendered vLLM/SGLang inference Deployments), kind cost pipeline (real cost-monitor against the pinned OpenCost + kube-prometheus-stack charts with Floci as the report bucket: data probe, ad-hoc Parquet report read back and column-checked, label-scoped OpenCost egress), K8s manifest validation, Lambda import checks, MCP server tests |
| `security.yml` | Security | bandit, pip-audit, npm audit across every owned package graph, bundler-audit on the committed gem graph, trivy (filesystem + per-image matrix), trufflehog, gitleaks, semgrep, checkov, KICS, CodeQL (Python + JavaScript) |
| `lint.yml` | Linting | actionlint, action SHA-pin verification (each version comment resolved against GitHub), hadolint, markdownlint, strict Zensical wiki build, mypy (strict + stacks + lambda), ruff (format + check, imports included), strict ShellCheck at `style` severity over every tracked `*.sh` path, yamllint |
...
| `pages.yml` | `workflow_run` after Unit Tests on `main` | Publish the project site to GitHub Pages: the Zensical wiki (`wiki/` + `zensical.toml`, staged by `scripts/build_wiki.py`) at the root with the generated API spec sheets at `/api/`, FastAPI's Swagger UI console per service at `/swagger/` (built from `docs/openapi/*.json` with the locked `swagger-ui-dist` self-hosted), the Python, Bash and Node.js HTML coverage reports at `/python-coverage/`, `/bash-coverage/` and `/nodejs-coverage/` (the Node.js one fetched from the Inference Streaming Proxy run for the same commit), and the three shields.io badge JSONs at the site root. Split out of Unit Tests so a Pages outage (or wiki build failure) can't fail the test gate; `lint:zensical:strict` runs the same build on PRs |
