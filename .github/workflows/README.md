...
| `lint.yml` | Linting | actionlint, action SHA-pin verification (each version comment resolved against GitHub), hadolint, markdownlint, strict MkDocs wiki build, mypy (strict + stacks + lambda), ruff (format + check, imports included), strict ShellCheck at `style` severity over every tracked `*.sh` path, yamllint |
