  ...
  "mcpServers": {
    "gco": {
      ...
      "args": [
        ...
        "3.14",
        "--from",
        "git+https://github.com/aws-solutions-library-samples/global-capacity-orchestrator-on-aws.git@v7.6.3",
        "gco-mcp"
      ],
      "env": {
...
> The `@v7.6.3` value is an exact reproducibility pin, not a version range. Update it deliberately when adopting a newer reviewed release. (Shell snippets can interpolate `${GCO_REF}`; JSON cannot, so the tag is written inline.)
>
  ...
  "mcpServers": {
    "gco": {
      ...
      "args": [
        ...
        "3.14",
        "--from",
        "gco-cli[cdk] @ git+https://github.com/aws-solutions-library-samples/global-capacity-orchestrator-on-aws.git@v7.6.3",
        "gco-mcp"
      ],
...
> **Optional metric-file formats.** Reading Parquet or TensorBoard `tfevents` metric files (via `metrics_from_shared_storage_file` / `metrics_from_local_file`) needs extra libraries, added the same way: `.[metrics-parquet]` (pandas + pyarrow), `.[metrics-tfevents]` (tbparse + tensorboard), or `.[metrics]` for both — e.g. `pip install -e ".[cdk,metrics,mcp]"`, or `gco-cli[cdk,metrics] @ git+…@v7.6.3` for the `uvx` form. Every other metric source ([CloudWatch](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/WhatIsCloudWatch.html), job logs, and JSON/CSV/JSONL/YAML/HF-Trainer-state files) works without them.
...
The `@v7.6.3` value is an exact release pin; update it deliberately when adopting a newer reviewed release.
...
Drop the `env` block if you do not need a [feature flag](#feature-flags). The `@v7.6.3` value is an exact release pin; update it deliberately when adopting a newer reviewed release.
...
```bash
claude mcp add gco -- uvx --python 3.14 --from "git+https://github.com/aws-solutions-library-samples/global-capacity-orchestrator-on-aws.git@v7.6.3" gco-mcp
```

Add a feature flag with `--env`:

```bash
claude mcp add --env GCO_ENABLE_INFRASTRUCTURE_DEPLOY=true gco -- uvx --python 3.14 --from "git+https://github.com/aws-solutions-library-samples/global-capacity-orchestrator-on-aws.git@v7.6.3" gco-mcp
```
...
The `@v7.6.3` value is an exact release pin; update it deliberately when adopting a newer reviewed release. For a local clone (development), swap the launch command for `python3 /absolute/path/to/global-capacity-orchestrator-on-aws/gco_mcp/run_mcp.py`.
...
```bash
GCO_REF=v7.6.3 # Current reviewed release; update deliberately.
uvx --python 3.14 --from "git+https://github.com/aws-solutions-library-samples/global-capacity-orchestrator-on-aws.git@${GCO_REF}" gco-mcp
```
...
Each `@v7.6.3` above is an exact release pin; update all of them together when adopting a newer reviewed release. For a clone-based client, swap `command` / `args` for the `python3 gco_mcp/run_mcp.py` form (plus `cwd` on Kiro) shown in [Setup](#setup); the `env` block is identical.
