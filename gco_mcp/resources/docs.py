...
EXAMPLE_METADATA: dict[str, dict[str, str | list[str]]] = {
    ...
    "kubeflow-trainjob": {
        "category": "Jobs & Training",
        "summary": "Kubeflow Trainer v2 TrainJob: 2-node distributed PyTorch through the torch-distributed runtime (CPU-sized; GPU variant documented via runtimePatches).",
        "gpu": "no",
        "opt_in": "",
        "submission": "gco jobs submit-sqs examples/kubeflow-trainjob.yaml --region us-east-1",
        "keywords": [
            "kubeflow",
            "trainer",
            "trainjob",
        ...
        "instance_types": [],
        "use_cases": [
            "multi-node training through the TrainJob API",
            "distributed PyTorch without writing JobSets",
            "gang-schedule training via the Kueue default queue",
        ],
        "related": ["multi-gpu-training", "efa-distributed-training", "kueue-job"],
    },
    "mlflow-tracking-job": {
        "category": "Jobs & Training",
        ...
        "use_cases": [
            ...
            "log metrics from a job",
            "verify the tracking pipeline end to end",
        ],
        "related": ["kubeflow-trainjob", "multi-gpu-training"],
    },
    "model-download-job": {
        "category": "Jobs & Training",
    ...
    "inference-vllm": {
        ...
        "gpu": "NVIDIA",
        "opt_in": "",
        "submission": "gco inference deploy my-llm -i vllm/vllm-openai:v0.29.0 --gpu-count 1",
        "keywords": [
            "vllm",
            "openai",
            "openai-compatible",
    ...
    "inference-triton": {
        ...
        "related": ["inference-vllm", "inference-sglang"],
    },
    "inference-sglang": {
        "category": "Inference Serving",
    ...
    "vector-store-search-job": {
        "category": "Caching & Databases",
        "summary": "Read-only semantic search against the built-in DynamoDB vector store: Bedrock query embedding + SearchVectors against the local replica, asserting at least one hit.",
        "gpu": "no",
        "opt_in": 'Vector store ("vector_store": {"enabled": true} in cdk.json)',
        "submission": "gco jobs submit-direct examples/vector-store-search-job.yaml -r us-east-1",
        "keywords": [
            "vector store",
            "dynamodb",
            "semantic search",
        ...
        "use_cases": [
            ...
            "RAG retrieval from a job",
            "query the vector store from workloads",
        ],
        "related": ["aurora-pgvector-job", "valkey-cache-job"],
    },
    ...
    "regional-shared-bucket-upload-job": {
        "category": "Storage & Persistence",
        "summary": "Uploads a file to the always-on Regional_Shared_Bucket using the gco-regional-shared-bucket ConfigMap via envFrom. Same-region as the cluster, so no cross-region egress — the right target for training checkpoints.",
        "gpu": "no",
        "opt_in": "",
        "submission": "gco jobs submit-direct examples/regional-shared-bucket-upload-job.yaml -r us-east-1",
        "keywords": [
            "s3",
            "regional bucket",
            "checkpoint",
        ...
        "instance_types": [],
        "use_cases": [
            "write training checkpoints to S3 from a job",
            "upload artifacts without cross-region egress",
            "persist job outputs to the region's own bucket",
        ],
        "related": ["cluster-shared-bucket-upload-job", "efs-output-job"],
    },
    "analytics-s3-upload-job": {
        "category": "Analytics",
    ...
    # --- Platform add-ons: GitOps, platform APIs, AWS resources as objects -----
    "argocd-gitops-job": {
        "category": "Platform Add-ons",
        "summary": "Argo CD Application that syncs a Job from Git into gco-jobs through the fenced gco-tenants project.",
        "gpu": "no",
        "opt_in": 'Argo CD ("helm": {"argocd": {"enabled": true}} in cdk.json)',
        "submission": "kubectl apply -f examples/argocd-gitops-job.yaml",
        "keywords": ["argocd", "argo cd", "gitops", "application", "git", "sync"],
        "instance_types": [],
        "use_cases": [
            "deploy tenant workloads from a Git repository",
            "continuously reconcile a directory into gco-jobs",
        ],
        "related": ["crossplane-batch-job", "kro-batch-job", "simple-job"],
    },
    "crossplane-batch-api": {
        "category": "Platform Add-ons",
        "summary": "Crossplane XRD + Composition defining a namespaced BatchJob API that composes a Job.",
        "gpu": "no",
        "opt_in": 'Crossplane ("helm": {"crossplane": {"enabled": true}} in cdk.json)',
        "submission": "(applied before crossplane-batch-job.yaml)",
        "keywords": ["crossplane", "xrd", "composition", "platform api", "go-templating"],
        "instance_types": [],
        "use_cases": [
            "define a self-service platform API",
            "compose Kubernetes workloads from a custom resource",
        ],
        "related": ["crossplane-batch-job", "kro-batch-api"],
    },
    "crossplane-batch-job": {
        "category": "Platform Add-ons",
        "summary": "Crossplane BatchJob composite resource; Crossplane composes and runs a Job of the same name.",
        "gpu": "no",
        "opt_in": 'Crossplane ("helm": {"crossplane": {"enabled": true}} in cdk.json)',
        "submission": "kubectl apply -f examples/crossplane-batch-job.yaml",
        "keywords": ["crossplane", "composite resource", "xr", "crossview", "platform api"],
        "instance_types": [],
        "use_cases": [
            "run a Job through a Crossplane platform API",
            "see a composition in the Crossview dashboard",
        ],
        "related": ["crossplane-batch-api", "kro-batch-job", "argocd-gitops-job"],
    },
    "kro-batch-api": {
        "category": "Platform Add-ons",
        "summary": "kro ResourceGraphDefinition defining a namespaced BatchJob API that composes a Job.",
        "gpu": "no",
        "opt_in": 'kro EKS Capability ("eks_capabilities": {"kro": {"enabled": true}} in cdk.json)',
        "submission": "(applied before kro-batch-job.yaml)",
        "keywords": ["kro", "resourcegraphdefinition", "rgd", "eks capability", "platform api"],
        "instance_types": [],
        "use_cases": [
            "define a custom API with kro",
            "compose Kubernetes workloads from a custom resource",
        ],
        "related": ["kro-batch-job", "crossplane-batch-api"],
    },
    "kro-batch-job": {
        "category": "Platform Add-ons",
        "summary": "kro BatchJob instance; the kro EKS Capability composes and runs a Job of the same name.",
        "gpu": "no",
        "opt_in": 'kro EKS Capability ("eks_capabilities": {"kro": {"enabled": true}} in cdk.json)',
        "submission": "kubectl apply -f examples/kro-batch-job.yaml",
        "keywords": ["kro", "instance", "eks capability", "composition", "platform api"],
        "instance_types": [],
        "use_cases": [
            "run a Job through a kro API",
            "try the AWS-managed kro capability",
        ],
        "related": ["kro-batch-api", "crossplane-batch-job", "ack-sqs-queue"],
    },
    "ack-sqs-queue": {
        "category": "Platform Add-ons",
        "summary": "Amazon SQS queue declared as a Kubernetes object and managed by the ACK EKS Capability.",
        "gpu": "no",
        "opt_in": 'ACK EKS Capability ("eks_capabilities": {"ack": {"enabled": true, "iam_policy_arns": [...]}} in cdk.json)',
        "submission": "kubectl apply -f examples/ack-sqs-queue.yaml",
        "keywords": ["ack", "aws controllers for kubernetes", "sqs", "queue", "eks capability"],
        "instance_types": [],
        "use_cases": [
            "create an AWS resource from Kubernetes",
            "manage an SQS queue alongside the workloads that use it",
        ],
        "related": ["kro-batch-job", "keda-scaled-job", "sqs-job-submission"],
    },
}
...
DOC_METADATA: dict[str, dict[str, str | list[str]]] = {
    ...
    "CLUSTER_SHARED_BUCKET": {
        ...
        "related": ["ANALYTICS", "ARCHITECTURE", "REGIONAL_SHARED_BUCKET"],
    },
    "REGIONAL_SHARED_BUCKET": {
        "summary": "Reference for the always-on Regional_Shared_Bucket — the per-region S3 bucket each cluster's job pods can read and write in their own region, with no cross-region egress.",
        "topics": ["storage", "concepts", "security"],
        "keywords": [
            "s3",
            "regional bucket",
            "checkpoint",
    ...
    "EKS_CAPABILITIES": {
        "summary": "Opt-in EKS Capabilities — the AWS-managed ACK and kro installations GCO can attach per regional cluster: capability IAM roles, ACK permission models (managed policies or IAM Role Selectors), the tenant RBAC kro composes workloads with, and live status/drift.",
        "topics": ["eks", "customization", "security", "cli"],
        "keywords": [
            "eks capabilities",
            "ack",
    ...
    "CROSSPLANE": {
        "summary": "Self-managed Crossplane v2 and the Crossview dashboard (cdk.json helm.crossplane, off by default): namespaced composite resources that compose tenant workloads in gco-jobs and gco-inference only, the go-templating composition function, no provider packages, and the dashboard over a private port-forward.",
        "topics": ["customization", "security", "cli", "eks"],
        "keywords": [
            "crossplane",
            "crossview",
            "composite resource",
        ...
        "related": ["EKS_CAPABILITIES", "GITOPS", "CUSTOMIZATION", "CLI"],
    },
    "CONCEPTS": {
    ...
    "GITOPS": {
        "summary": "Self-managed Argo CD (cdk.json helm.argocd, off by default): namespaced mode fenced to gco-jobs and gco-inference by the gco-tenants AppProject and RBAC, the optional GitOps hand-off to one repository path per cluster, the repo-server autoscaler, private repositories, and the UI over a private port-forward.",
        "topics": ["customization", "security", "cli", "eks"],
        "keywords": [
            "argo cd",
            "argocd",
        ...
        "related": ["EKS_CAPABILITIES", "CROSSPLANE", "CUSTOMIZATION", "CLI"],
    },
    "IMAGE_MIRROR": {
    ...
    "UPGRADING": {
        "summary": "Upgrading a deployment with gco upgrade — checks out the latest release tag (preserving cdk.json), refreshes the local install and dev image, scales the workload tier to zero and redeploys; what is destroyed, what to back up to the cluster-shared bucket first, and how to restore it.",
        "topics": [
            "deployment",
            "customization",
            "cli",
        ...
        "related": ["MAINTENANCE", "CUSTOMIZATION", "CLUSTER_SHARED_BUCKET", "CLI"],
    },
    "VOLCANO": {
