...
Three thin wrappers over the existing ``audit_logger`` from ``gco_mcp/audit.py``.
...
The Mission package is loaded via ``sys.path``-on-``gco_mcp/`` (the same trick
...
def _now_iso() -> str:
    ...
    ``emit_startup_log`` in ``gco_mcp/audit.py``.
    """
