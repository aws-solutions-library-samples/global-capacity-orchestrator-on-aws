...
class DynamoDBBackend:
    ...
    ``gco_mcp/`` free of the forbidden ``mcp -> cli`` import edge while
...
# the ``GCO_MCP_TOOL_SEARCH`` precedent in ``gco_mcp/server.py``.
...
def get_backend() -> MissionStateBackend:
    ...
    for ``GCO_MCP_TOOL_SEARCH`` in ``gco_mcp/server.py``. The resolved
