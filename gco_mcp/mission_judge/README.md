...
`gco_mcp/mission_judge/` is the dependency-light core behind GCO's read-only
...
[`gco_mcp/tools/semantic_progress.py`](../tools/semantic_progress.py), which reuses
...
These two live in the wrapper, [`gco_mcp/tools/semantic_progress.py`](../tools/semantic_progress.py):
...
- **Pure core, thin wrapper.** FastMCP, the sampling transport, and the event
  loop live in `gco_mcp/tools/semantic_progress.py`; the scoring logic lives here.
...
- [`gco_mcp/tools/semantic_progress.py`](../tools/semantic_progress.py) — the
...
- [`gco_mcp/metric_readers/README.md`](../metric_readers/README.md) — the sibling
