...
| GitHub Pages URL | `aws-solutions-library-samples.github.io/global-capacity-orchestrator-on-aws` | The published site: orientation wiki at the root, coverage reports at `/python-coverage/`, `/bash-coverage/` and `/nodejs-coverage/`, badge JSON at `/python-coverage-badge.json` and its two siblings (in `zensical.toml`, `wiki/*.md`, the README badges, and the wiki guard test) |
...
`https://<owner>.github.io/<repo>/`: the orientation wiki (built with Zensical
...
(`zensical.toml` `site_url`/`repo_url`, the nav's coverage-report links, and every
