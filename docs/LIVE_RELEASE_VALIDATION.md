...
| `network-posture` | `topology` | Prove the shipped NetworkPolicies decide traffic on the live cluster: from run-labelled, digest-pinned probe Jobs, same-namespace `gco-jobs` traffic, the inference-monitor's `:9090/metrics`, HTTPS egress from `gco-jobs`, and the node's EKS Pod Identity Agent (`http://169.254.170.23/v1/credentials`, the credential path behind `gco-service-account`) from `gco-jobs` must be reachable, while cross-namespace ingress into `gco-jobs` and `gco-system` and port-80 egress from `gco-jobs` to the internet must be blocked. Each probe reports the steady state of its connection (one answer held for 30 seconds, after at least 45 seconds), because the VPC CNI admits a new pod's traffic until its policies attach; the changes of answer it saw are kept as evidence. A probe an interruption evicted is re-run once, and a listener that did not last the matrix fails the action by name. Deny verdicts are recorded as skipped when cdk.json sets `eks_cluster.network_policy_enforcement: false`; every probe and listener is deleted before the action returns |
| `eks-capabilities` | `topology` | Prove the opt-in [EKS Capabilities](EKS_CAPABILITIES.md) the run deployed with: every enabled AWS-managed capability (ACK, kro) attached to each Region's cluster and `ACTIVE` with no drift from the configuration, judged by the same merge `gco stacks capabilities status` prints. AWS API only; what the capabilities do inside the cluster, and the self-managed Argo CD and Crossplane add-ons, are proved by the [example harness](EXAMPLE_VALIDATION.md). Passes with a note when no type is enabled |
...
The smoke Jobs behind `api`, `sqs`, `central-queue` and `schedulers` (`scripts/live_release_validation/manifests/*-smoke-job.yaml`) run with `backoffLimit: 0`, so a workload that exits non-zero fails its action on the spot, plus a `podFailurePolicy` that ignores a pod carrying the `DisruptionTarget` condition. That condition is what an Eviction API drain leaves behind, and EKS Auto Mode consolidates nodes while the run is in progress — the inference matrix leaves spare capacity behind it, and one run lost `central-queue` to a busybox pod that was bound to a node two seconds before Auto Mode began draining it, evicted while running, and counted as the Job's single allowed failure even though its container had already exited 0. With the policy the Job replaces a disrupted pod instead. The network-posture probe and listener Jobs carry no such policy, because their verdicts depend on one pod's uninterrupted connection and a quietly replaced pod would hide the disruption. They carry `karpenter.sh/do-not-disrupt` and prefer on-demand capacity instead, and the action handles what still gets through: a later run lost the HTTPS egress probe to an Auto Mode interruption 46 seconds into its sampling and then waited out the full pod timeout on a pod that no longer existed. Now a probe whose Job failed with no pod left, or whose pod carries `DisruptionTarget` without a verdict exit code, is read at once and re-run once from a fresh Job, and a second disruption is reported as one. A listener or inference-monitor pod that did not last the matrix fails the action by name, since every verdict dialed against it is void.
...
Add `--eks-capabilities all` (or a comma list of `ack`, `kro`) when the release touches the [EKS Capabilities](EKS_CAPABILITIES.md) surface — the regional stack's capability roles and their managed-policy attachments, the `AWS::EKS::Capability` resources, the kro tenant RBAC in `07-kro-tenant-access.yaml`, or `gco stacks capabilities`. The run then enables the named types through the run-scoped `eks_capabilities_overrides` CDK context (a JSON object deep-merged over the `cdk.json` block, never an edit to it) so the `eks-capabilities` action proves them; nothing has to exist in the account beforehand. The override becomes part of the checkpoint identity, so a `--resume` must repeat it exactly. Argo CD and Crossplane are Helm charts now, not capabilities: prove them with the [example harness](EXAMPLE_VALIDATION.md) (`examples/argocd-gitops-job.yaml`, `examples/crossplane-batch-job.yaml`), which enables each chart for its own run.
