...
| `eks-capabilities` | `topology` | Prove the opt-in [EKS Capabilities](EKS_CAPABILITIES.md) the run deployed with: every enabled AWS-managed capability (Argo CD, ACK, kro) attached to each Region's cluster and `ACTIVE` with no drift from the configuration (the same merge `gco stacks capabilities status` prints); for Argo CD, EKS publishing the hosted server URL and, through the tunnelled kubectl session, the `local-cluster` Secret registering the cluster by ARN and the RBAC bound to the capability role's access-entry group; for the GitOps hand-off, the fenced `AppProject` allowing the run's repository and the root `Application` reaching `Synced`/`Healthy` at the run's commit within a bounded wait, with the fixture ConfigMap from [`examples/gitops/tenant-smoke`](../examples/gitops/tenant-smoke/) present in `gco-jobs` carrying Argo CD's tracking label. Passes with a note when no capability is enabled (the shipped default); `--eks-capabilities` with the Identity Center inputs enables them for the run |
...
Add `--eks-capabilities all` (or a comma list of `argocd`, `ack`, `kro`) when the release touches the [EKS Capabilities](EKS_CAPABILITIES.md) surface — the regional stack's capability roles and `AWS::EKS::Capability` resources, the Argo CD manifests `07-argocd-cluster-access.yaml` / `08-argocd-gitops.yaml`, or `gco stacks capabilities`. The run then enables the named types through the run-scoped `eks_capabilities_overrides` CDK context (a JSON object deep-merged over the `cdk.json` block, never an edit to it) so the `eks-capabilities` action proves them. Argo CD has two operator inputs with no defaults, because the hosted control plane authenticates only through IAM Identity Center: `--argocd-idc-instance-arn` (the instance ARN) and at least one `--argocd-identity SSO_USER:<id>` or `SSO_GROUP:<id>`, which is granted the Argo CD `ADMIN` role for the run. The GitOps hand-off is on by default and points Argo CD at this checkout's `origin` remote (as HTTPS; override with `--argocd-gitops-repo-url`) at the run's `--expected-sha` (`--argocd-gitops-revision`), path `examples/gitops/tenant-smoke` (`--argocd-gitops-path`), with an automated sync policy (`--argocd-gitops-sync-policy`); the commit under validation must therefore be pushed to that remote before the run, and `--no-argocd-gitops` proves the capability alone. The override becomes part of the checkpoint identity, so a `--resume` must repeat it exactly. To capture the docs screenshot of the hosted Argo CD UI (`images/argocd-ui.png`), select the actions up to `eks-capabilities` without `destroy`, run `gco stacks capabilities argocd screenshot -o images/argocd-ui.png` against the live cluster, then `--resume` the run with `--actions all` so `destroy` and `final-inventory` complete it.
