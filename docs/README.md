...
| [Autopilot](AUTOPILOT.md) | Everyone | `gco autopilot` — one command to Claude Code by default, or `gco autopilot --engine codex` for OpenAI Codex; both use [Amazon Bedrock](https://docs.aws.amazon.com/bedrock/latest/userguide/what-is-bedrock.html) with the GCO MCP server and companion MCPs wired in |
