...
| [Autopilot](AUTOPILOT.md) | Everyone | `gco autopilot` — one command to Claude Code by default, `gco autopilot --engine codex` for OpenAI Codex, or `gco autopilot --engine opencode` for OpenCode; every engine uses [Amazon Bedrock](https://docs.aws.amazon.com/bedrock/latest/userguide/what-is-bedrock.html) with the GCO MCP server and companion MCPs wired in |
| [Inference Guide](INFERENCE.md) | ML engineers | Deploy and manage multi-region GPU inference endpoints ([vLLM](https://docs.vllm.ai/en/latest/), [SGLang](https://docs.sglang.ai/), [Triton](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/index.html)) |
...
| [EKS Capabilities](EKS_CAPABILITIES.md) | Platform teams | Opt-in AWS-managed [ACK](https://aws-controllers-k8s.github.io/docs/) and [kro](https://kro.run/) per regional cluster (off by default): capability IAM roles with exactly the permissions you configure, the tenant RBAC kro composes workloads with, and live status/drift |
| [GitOps with Argo CD](GITOPS.md) | Platform teams | Self-managed [Argo CD](https://argo-cd.readthedocs.io/en/stable/) per regional cluster (off by default): namespaced and fenced to the tenant namespaces, an optional GitOps hand-off to one repository path per cluster, a repo-server autoscaler, and the UI over a private port-forward |
| [Crossplane](CROSSPLANE.md) | Platform teams | Self-managed [Crossplane](https://docs.crossplane.io/) v2 and the Crossview dashboard (off by default): your own Kubernetes APIs whose composite resources compose tenant workloads in `gco-jobs` and `gco-inference` only |
...
| [Swarm](SWARM.md) | Operators | Supervise concurrent child Missions under explicit fleet, concurrency, and pooled iteration budgets |
...
| [Regional Shared Bucket](REGIONAL_SHARED_BUCKET.md) | Operators | Always-on per-region [S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Welcome.html) bucket each cluster's job pods read and write in their own region — no cross-region egress, the right target for checkpoints |
...
| [openapi/](openapi/) | Generated OpenAPI documents for every HTTP surface — the four services (`scripts/generate_openapi.py`), the two AWS API Gateways read out of the synthesized CDK stacks (`scripts/generate_api_gateway_openapi.py`) and the in-cluster Gateway composed from its HTTPRoute (`scripts/generate_cluster_gateway_openapi.py`); rendered into spec sheets, an interaction diagram and Swagger UI consoles under [`diagrams/api_specs/`](../diagrams/api_specs/README.md) and on the project site |
...
4. [EKS Capabilities](EKS_CAPABILITIES.md), [GitOps with Argo CD](GITOPS.md) and [Crossplane](CROSSPLANE.md) — opt into AWS-managed ACK and kro, reconcile tenant workloads from Git, and publish your own platform APIs
