...
| [EKS Capabilities](EKS_CAPABILITIES.md) | Platform teams | Opt-in AWS-managed [Argo CD](https://argo-cd.readthedocs.io/en/stable/), [ACK](https://aws-controllers-k8s.github.io/docs/) and [kro](https://kro.run/) per regional cluster (off by default), the fenced GitOps hand-off that points Argo CD at a GCO-managed per-cluster CodeCommit repository (`gitops push`) or your own, the Identity Center bootstrap for Argo CD sign-in, and the `gco stacks capabilities` CLI including the hosted Argo CD UI |
