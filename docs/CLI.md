...
| Option | Short | Description |
|--------|-------|-------------|
| `--region` | `-r` | Check a single region |
| `--all-regions` | `-a` | Check every configured region (the default) |
...
| Option | Short | Description |
|--------|-------|-------------|
| `--instance-type` | `-i` | Instance type (required) |
| `--region` | `-r` | Region (required) |
| `--fault-tolerance` | `-f` | Fault tolerance level: `high`, `medium` (default), or `low` |
...
gco capacity ai-recommend -w "ML training" --model us.anthropic.claude-sonnet-4-6
```
...
```bash
gco capacity traffic-dial show
gco --output json capacity traffic-dial show   # via the global --output flag
```
...
|--------|-------|-------------|
| `--region` | `-r` | AWS region (required) |
| `--namespace` | `-n` | Kubernetes namespace (default: `gco-jobs`) |
| `--storage-type` | `-t` | Storage type: `efs` or `fsx` (default: `efs`) |
| `--pvc` | | PVC to list (default: `gco-shared-storage` for EFS) |
...
| `--namespace` | `-n` | Kubernetes namespace (default: `gco-jobs`) |
| `--storage-type` | `-t` | Storage type: `efs` or `fsx` (default: `efs`) |
| `--pvc` | | PVC to read from (default: `gco-shared-storage` for EFS) |
...
| `--accelerator` | | Accelerator type: `nvidia` for GPU instances (default), `neuron` for [Trainium](https://aws.amazon.com/ai/machine-learning/trainium/)/Inferentia |
...
| `--namespace` | `-n` | Fallback namespace for manifests that don't declare their own (manifest `metadata.namespace` takes precedence) |
| `--label` | `-l` | Add labels (`key=value`, repeatable) |
...
| `--since` | `-s` | Hours to look back in CloudWatch (default: 24) |
...
gco -o json jobs policy -r us-east-1 | jq '.policy.trusted_registries'
```
...
- `--tool-allowlist NAME` — Repeatable. Tools the loop may invoke; required unless `--allow-all-tools` is set. The first allowlisted tool also seeds the deterministic strategy when sampling is off.
- `--allow-all-tools` — Resolve the session's tool allowlist to every registered MCP tool (minus the `mission_*` control tools). Mutually exclusive with `--tool-allowlist`.
...
- `--dry-run` — Use a stub tool dispatcher and disable Strategy_Revision sampling during iteration (the criteria scaffolder still runs through Bedrock when sampling is enabled). Smoke-tests the loop without spending AWS or Bedrock credits.
...
- `--tool-allowlist NAME` — Repeatable. Tools the loop may invoke; required unless `--allow-all-tools` is set.
- `--allow-all-tools` — Resolve the session's tool allowlist to every registered MCP tool (minus the `mission_*` control tools). Mutually exclusive with `--tool-allowlist`.
- `--cadence` — Checkpoint cadence kind (default: `every_iteration`).
...
- `--dry-run` — With `--run`: use a stub tool dispatcher and disable Strategy_Revision sampling while iterating.
...
- `--dry-run` — Use a stub tool dispatcher and disable Strategy_Revision sampling.
- `--output table|json` — Output format (default: `json`).
...
- `--include-observations` — With `--format full`: include each iteration's observation and strategy dicts (what every tool returned and what strategy was proposed).
- `--output table|json` — Output format (default: `json`).
...
|--------|-------|-------------|
| `--region` | `-r` | Filter by region |
| `--cluster` | | EKS cluster name (default: `<project_name>-<region>`) |
...
| `--fallback-on-demand` | | Fall back to on-demand capacity when the reservation is exhausted |
| `--efa` | | Enable EFA support (adds the EFA taint and labels for p4d/p5 instances) |
...
# The full document for scripts and agents (--output is a global flag: it goes before the command)
gco --output json status
...
- `-k, --keep INTEGER` — Number of most-recent tasks to keep (default: 50)
