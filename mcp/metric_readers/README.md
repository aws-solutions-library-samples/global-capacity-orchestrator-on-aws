...
`mcp/metric_readers/` is the dependency-light core behind GCO's read-only
...
[`mcp/tools/metrics.py`](../tools/metrics.py). That separation is the single
...
`aggregation` docstrings in [`mcp/tools/metrics.py`](../tools/metrics.py) so the
...
docstring in [`mcp/tools/metrics.py`](../tools/metrics.py).
...
| `_TAIL_MIN` / `_TAIL_MAX` / `_TAIL_DEFAULT` | [`mcp/tools/metrics.py`](../tools/metrics.py) | Job-log tail clamp range and default. |
| `_MAX_BYTES_DEFAULT` | [`mcp/tools/metrics.py`](../tools/metrics.py) | File reader size cap (default 10 MiB). |
...
3. Add a thin `@mcp.tool` + `@audit_logged` wrapper in
   [`mcp/tools/metrics.py`](../tools/metrics.py) that calls your module, runs
...
5. Add a row to the Metrics table in [`mcp/README.md`](../README.md#metrics) and
   update the count in [`mcp/tools/README.md`](../tools/README.md).

## Design Principles

- **Pure core, thin wrapper.** Everything here is free of FastMCP, the event
  loop, and ambient state. The wrapper in `mcp/tools/metrics.py` owns I/O,
...
- [`mcp/tools/metrics.py`](../tools/metrics.py) — the tool wrappers that call
  this package.
