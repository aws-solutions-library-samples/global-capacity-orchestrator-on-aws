...
* ``mcp://gco/feature-flags`` — the umbrella + per-tool flag table.
  ...
  each ``mcp/tools/*.py`` module. The ``mission`` family lives in a
...
walking each ``mcp/tools/*.py`` module on every list call — would
...
_TOOL_GATING_TABLE: dict[str, str] = {
    # mcp/tools/capacity.py — module-level gate for purchase tools
    "reserve_capacity": FLAG_CAPACITY_PURCHASE,
    # mcp/tools/models.py — model upload + destructive
    "models_upload": FLAG_MODEL_UPLOAD,
    "delete_model": FLAG_DESTRUCTIVE_OPERATIONS,
    # mcp/tools/images.py — image-publish + destructive
    ...
    # mcp/tools/stacks.py — deploy + destroy
    ...
    # mcp/tools/mission.py — module-level gate
