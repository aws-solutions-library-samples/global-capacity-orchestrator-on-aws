...
<details>
<summary>OpenCode Autopilot recording (click to expand)</summary>

![GCO Autopilot with OpenCode](autopilot-opencode.gif)

*A real `gco autopilot --engine opencode --no-companions` session on Amazon Bedrock (Moonshot AI's Kimi K3), recorded with a least-privilege profile: only the GCO MCP `find_docs`/`read_resource` tools, every built-in OpenCode tool denied through OpenCode's own `OPENCODE_CONFIG_CONTENT` permission override, and any permission prompt fails the recording ([docs](../docs/AUTOPILOT.md) · [re-record](record_autopilot.sh), `DEMO_ENGINE=opencode DEMO_MODE=live`).*
...
| `record_autopilot.sh` | Records the `gco autopilot` demo as an animated GIF for any engine (`DEMO_ENGINE=claude-code`, `codex`, or `opencode`) — a real Bedrock-backed answer by default (`DEMO_MODE=live`), or a credential-free launch-plan recording (`DEMO_MODE=plan`) |
...
| `autopilot-opencode.cast` | Asciinema recording of the OpenCode Autopilot demo (generated with `DEMO_ENGINE=opencode`) |
| `autopilot-opencode.gif` | Animated GIF of the OpenCode Autopilot demo (generated with `DEMO_ENGINE=opencode`, embedded above) |
...
The security workflow fully decodes the six tracked GIFs and enforces reviewed
